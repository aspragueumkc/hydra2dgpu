...
### Shared vs GUI-only services

A service lives in `swe2d/services/` when it:
- Has zero PyQt5 imports
- Has zero widget access
- Is consumed by both GUI and CLI (or could be)
- Contains pure logic: numpy, sqlite3, netCDF4, matplotlib(Agg)

A service stays in `swe2d/workbench/services/` when it:
- Depends on QGIS project state (QgsProject)
- Reads/manages widget state
- Is only relevant to the GUI workflow

### CLI is NOT a workbench consumer

The CLI at `swe2d/cli/` MUST import only from:
- `swe2d/services/`
- `swe2d/runtime/`
- `swe2d/mesh/`
- `swe2d/results/`
...
The CLI MUST NOT import from `swe2d/workbench/`.
...
grep -n "view\._model_tab_view\.\|view\._map_tab_view\.\|view\._topo_widgets\.\|view\._results_toolbox\.\|view\._run_dock\." \
  swe2d/workbench/controllers/*.py | grep -v "getattr\|is None"

# VIOLATION: controller accessing results/run dock internals (use typed protocols)
grep -n "view\._results_toolbox\.\|view\._run_dock\.\|view\._model_tab_view\." \
  swe2d/workbench/controllers/run_controller.py

# CORRECT (controller uses typed protocol getters):
#   view.model_tab.get_rain_update_interval_s()
#   view.results_toolbox.refresh_run_list()
#   view.run_dock.set_run_button_enabled(True)
...
## Typed View Protocols

This repo defines typed view protocols in `swe2d/workbench/views/view_protocols.py`:

| Protocol | View class | Used by |
|----------|-----------|---------|
| `ModelTabViewProtocol` | `ModelTabView` | Controllers (via `view.model_tab`) |
| `ResultsToolboxProtocol` | `ResultsToolbox` | View helpers (via `dialog.results_toolbox`) |
| `MapTabViewProtocol` | `MapTabView` | Controllers (via `view.map_tab`) |
| `RunDockProtocol` | `RunDockWidget` | Controllers (via `view.run_dock`) |

The `RunView` protocol in `swe2d/workbench/controllers/protocols_controller.py` exposes typed properties (`model_tab`, `results_toolbox`, `run_dock`) so controllers access sub-views through typed getters, never through direct widget paths.

```python
# CORRECT (controller):
view.model_tab.get_rain_update_interval_s()  # typed protocol getter
view.results_toolbox.refresh_run_list()       # typed protocol method

# WRONG (controller):
view._model_tab_view.rain_update_interval_spin.value()  # reaches through to widget
view._results_toolbox._rebuild_run_list()               # direct widget access
```
...
```bash
# 1. No Qt in shared service layer
...
# 2. No command-line Qt in service layer
! grep -q 'from PyQt' swe2d/workbench/services/ && echo "PASS: workbench services have no Qt imports"
...
# 5. No CLI imports from workbench
! grep -r 'from swe2d\.workbench' swe2d/cli/ --include="*.py" 2>/dev/null && echo "PASS: CLI does not depend on workbench"
```

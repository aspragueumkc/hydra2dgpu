# SWE2D GPU Solver, Coupling, and Studio UI — Architecture Report

> **Generated**: 2026-06-04  
> **Scope**: GPU solver core, rainfall/infiltration, hydraulic structures, urban drainage coupling, QGIS Studio UI  
> **Design philosophy**: GPU-first — CPU paths exist for fallback/debug only; GPU is the engineering priority

---

## 1. Overview

The SWE2D GPU path is an HLLC finite-volume shallow-water solver with selectable higher-order spatial reconstruction (schemes 0–5) and multi-stage Runge-Kutta time integration (RK2/4/5/6). It is extended by three coupling submodules — rainfall infiltration, hydraulic structures, and 1D urban drainage networks — orchestrated through a single `SWE2DCouplingController`. The entire feature surface is exposed to the user through the QGIS Studio UI, a 7-tab Qt dialog backed by `.ui` files from Qt Designer.

```mermaid
graph TD
    subgraph "QGIS Studio UI"
        A["swe2d_workbench_qt.py<br/>7 tabs + feature toggles"]
    end
    ...
    A -->|"PipeNetworkConfig<br/>HydraulicStructureConfig<br/>RainFieldConfig"| B
...
- **GPU-first, not CPU parity** — The CPU path is kept for validation and fallback. The GPU path drives all design decisions. AGENTS.md explicitly states: "Current SWE2D engineering priority is CUDA optimization and robustness hardening, not CPU parity."
...
swe2d/runtime/backend.py: SWE2DBackend.set_rain_cn_forcing_native()
    ├─ CPU path: swe2d_solver.cpp → build_solver_rain_cn_source()
...
Both CPU and GPU implement identical NRCS SCS CN infiltration:
...
| **CPU/GPU CN formula drift** | Low | Both paths implement the same formula, but floating-point differences between host (x86) and device (CUDA) can accumulate over long simulations. |
...
swe2d/runtime/coupling.py: SWE2DCouplingController
    ...
    ↓
pybind11: swe2d_gpu_compute_structure_flows(...) [CPU/GPU helper]
    OR
...
**Constraint**: This path is only active when `coupling_loop == "cuda"` AND drainage is `None` AND no bridge structures are enabled. Otherwise, the controller falls back to `compute_source_rates()` with host-side WSE reads.
...
- Bridge CUDA coupling is controlled by `bridge_cuda_coupling` flag and `bridge_stacked_coupling_mode` (legacy_scalar vs phase3_spatial)
...
| **Bridge CUDA coupling feature-gated** | Medium | Bridge coupling on GPU requires `bridge_cuda_coupling=True`. When False, bridge flows are computed on CPU even in `coupling_loop="cuda"` mode, forcing a host-device sync. |
...
| **Redistribution fallback degrades silently** | Medium | If the CUDA redistribution kernel fails (e.g., OOM), `_apply_redistribution()` falls back to a Python loop with only a log message. The caller receives normally-shaped output but with point-source injection — no exception is raised. |
...
| **Pipe-end loss coefficient defaults** | Medium | `pipe_end_inlet_loss_k` defaults to 0.5 and `pipe_end_outlet_loss_k` to 1.0. These are reasonable for square-edged entrances/exits but are wrong for rounded, flared, or projecting inlets. No warning when defaults are used. |
| **GPU drainage blocked with structures** | Medium | `apply_native_device_sources()` returns `False` when `self.drainage is not None`, forcing the host-based `compute_source_rates` path. The GPU drainage `swe2d_gpu_drainage_step` kernel exists but is not integrated into the persistent on-device fast path. |
...
class SWE2DCouplingController:
    def __init__(
        ...
        coupling_loop: str = "cpu",          # "cpu" | "cuda"
        drainage_solver_backend: str = "cpu", # "cpu" | "gpu"
...
compute_source_rates(t_s, dt_s, h, hu, hv)
    │
    ├─ coupling_loop == "cuda" AND native CUDA available?
    │   └─ YES → _compute_source_rates_cuda()
    ...
    │
    └─ NO → CPU path
        ├─ drainage.solve_network_step(dt) + surface_exchange_source_rate()
        ├─ _native_structure_flows(..., use_cuda=False) OR Python structure_flows
        └─ _structure_source_rate_from_flows() → source rate array
```
...
| Issue | Severity | Detail |
|---|---|---|
| **Fast path silently skipped** | High | When drainage or bridges are active, `apply_native_device_sources()` returns `False` with no diagnostic. The caller falls back to `_compute_source_rates_cuda()` which involves host-side WSE reads. There is no log message indicating the degraded path. |
| **`__pycache__` stale module hazards** | High | Per AGENTS.md: "After any structural change to a Python module… always purge `__pycache__` before the user restarts QGIS." The coupling controller crosses many module boundaries (`coupling.py` → `backend.py` → `bindings.cpp`). Stale `.pyc` files cause invisible failures (wrong arity, missing attributes, silent fallback) that are not caught by normal error handling. |
| **Redistribution fallback degrades silently** | Medium | If the CUDA redistribution kernel fails, `_apply_redistribution()` falls back to single-cell injection. The only indication is a red `[ERROR]` log message. The solver continues with point-source injection, producing physically different results with no exception. |
...
### 7.1 Dual UI Architecture

The SWE2D workbench has two parallel UI paths that must stay in sync:
...
Seven tab pages, each loaded from a dedicated `.ui` file in `forms/`:

| Tab | `.ui` File | Key Bind Method |
|---|---|---|
| Mesh | `forms/swe2d_mesh_tab.ui` | `_bind_mesh_tab_controls()` |
| Map | `forms/swe2d_map_tab.ui` | `_bind_map_tab_data_controls()` |
| Topology | `forms/swe2d_topology_tab.ui` | `_bind_topology_tab_controls()` |
| Boundary | `forms/swe2d_boundary_tab.ui` | `_bind_boundary_tab_controls()` |
| **Model** | `forms/swe2d_model_tab.ui` | Multiple (see below) |
| 3D Patch | `forms/swe2d_3d_patch_tab.ui` | `_bind_3d_patch_tab_controls()` |
| Run | `forms/swe2d_run_tab.ui` | `_bind_run_tab_controls()` |

The **Model tab** contains 3 `QToolBox` pages within a single `.ui`:

| ToolBox Page | Form Name | Content |
|---|---|---|
| Solver Parameters | `model_solver_form` | Time integrator, spatial scheme, CFL, GPU toggle |
| Rain / Hydrology | `model_rain_form` | Hyetograph, CN defaults, extreme-rain subcycling |
| Structures & Drainage | `model_drain_form` | Culvert solver mode, coupling loop, drainage backend |

### 7.3 Feature Toggle Mechanism (4-File Pattern)

Feature toggles control visibility of GPU, rainfall, drainage/structures, and 3D-patch features:

#### File 1: Feature Flags Dictionary
`swe2d_workbench_qt.py` (line ~10388):
```python
...
def _studio_feature_keywords(self) -> Dict[str, Tuple[str, ...]]:
    return {
        ...
        "drainage_structures": (
            "drain", "node", "link", "inlet", "outfall", "pipe",
            "network", "structure", "culvert", "weir", "orifice",
            "gate", "spillway", "coupling",
        ),
...
#### File 4: Menu & Toolbar Actions
`swe2d/workbench/extracted/studio_host_methods.py` (line ~259):
- **Menu**: "Enable Rainfall", "Enable Drainage & Structures", "Enable 3D Patch" (checkable)
...
### 7.5 Widget Binding Convention

Widgets in `.ui` files are accessed through `_find_or_create_*` helper methods:

```python
self.widget_name = _find_or_create_double_spin_box(
    name="widget_name",
    parent_form=form,
    label="Display Label",
)
...
The `tools/ui_bind_sync.py` tool verifies binding coverage:

```bash
# Check for missing bindings
python tools/ui_bind_sync.py forms/swe2d_model_tab.ui swe2d_workbench_qt.py --missing

# Remove orphaned Python bindings for deleted widgets
python tools/ui_bind_sync.py forms/swe2d_model_tab.ui swe2d_workbench_qt.py
```

### 7.6 UI → Backend Data Flow

```
[QGIS Layers: swe2d_mesh, swe2d_structures, swe2d_drainage_nodes, ...]
    ↓
UI widgets read layer features + user parameters
    ↓
swe2d/workbench/extracted/model_and_run_methods.py
    ├─ _bind_model_tab_hydrology_controls() → RainFieldConfig
    ├─ _bind_model_tab_solver_controls() → SWE2DSolverParams
    └─ _bind_model_tab_3d_subgrid_drainage_controls()
        ├─ PipeNetworkConfig (from drainage node/link/inlet/outfall layers)
        └─ HydraulicStructureConfig (from structures layer)
    ↓
SWE2DCouplingController(cell_area, cell_bed,
    drainage=SWE2DUrbanDrainageModule(cfg=pipe_network_config),
    structures=SWE2DStructureModule(cfg=structures_config),
    coupling_loop="cuda", drainage_solver_backend="gpu", ...)
    ↓
SWE2DBackend.run(..., source_rate_callback=coupling_controller.source_rate_callback())
```

### 7.7 Potential Issues
...
| `cpp/src/swe2d_solver.cpp` | CPU rainfall (`build_solver_rain_cn_source`), BC hydrograph application |
...
| `swe2d/runtime/backend.py` | `SWE2DBackend.set_rain_cn_forcing_native()` — dispatch to GPU/CPU CN setup |
...
| `forms/swe2d_mesh_tab.ui` | Mesh tab layout |
| `forms/swe2d_map_tab.ui` | Map tab layout |
| `forms/swe2d_boundary_tab.ui` | Boundary conditions tab |
| `forms/swe2d_run_tab.ui` | Run tab |
| `tools/ui_bind_sync.py` | Widget binding verification + orphan cleanup |

### Documentation

| File | Role |
|---|---|
| `AGENTS.md` | GPU validation priority, Godunov rollout handoff, `__pycache__` discipline, Studio UI structural change rules |
| `docs/STUDIO_UI_ARCHITECTURE.md` | Step-by-step checklists for tab/form/widget changes |

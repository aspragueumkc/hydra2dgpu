...
## Session: Thu Jul 09 2026 — Pipe-End Exchange Fix

### Goal
Fix pipe-end exchange so the downstream outfall cell receives flow from the pipe in the GPU coupling path, including the `example_test_project.gpkg` scenario.

### Root Cause
The diffusion-wave pipe solver only applied friction/minor-loss source terms to the discharge update. With `cell_Q` initialized to zero, those terms are zero, so `cell_Q` stayed zero forever. The pipe-end exchange kernel was updated to use `node_net_q` from the solver's mass balance, but `node_net_q` was also zero because it accumulates `cell_Q`. Only the fully-dynamic solver self-started because it includes a pressure-gradient term.

### Changes
- `cpp/src/pipe1d.cu` / `cpp/src/pipe1d.cuh`
  - Added node topology arrays (`cell_from_node`, `cell_to_node`, `node_invert`, `node_depth`) to the diffusion-wave kernel and host wrapper.
  - Added a pressure-gradient source term `-g * A * dH/dx` to the diffusion-wave momentum update so it self-starts from zero discharge.
  - Fixed the exchange kernel to declare `q_net` as mutable (`double` instead of `const double`) so the surface-availability limiter can clamp it.
- `tests/test_swe2d_pipe1d.py`
  - Updated `test_dry_pipe_no_change` to call `swe2d_pipe1d_init_area_from_depth` after uploading zero node depths, so the dry pipe has zero area and stays quiescent.
- Rebuilt `hydra_swe2d` native extension.

### Verification
- `tests/test_swe2d_pipe1d.py` + `tests/test_swe2d_gpu_drainage_network.py` → 25 passed
- `tests/test_coupling_integration.py` + `tests/test_swe2d_gpu_coupling_kernel.py` + `tests/test_coupling_rain_matrix.py` + `tests/test_swe2d_gpu_validation_perf.py` + `tests/test_swe2d_gpu_unstructured.py` → 20 passed, 4 skipped
- `python -m py_compile` on modified Python test files → OK
- Purged `__pycache__`

### Notes
- The pipe-end exchange kernel now uses `node_net_q`, which is correct for both solver modes after the diffusion-wave fix.

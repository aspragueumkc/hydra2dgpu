...
┌──────────────────────────────────────────────────────────┐
│                   QGIS Plugin Layer                       │
│  hydra_plugin.py, swe2d_workbench_qt.py (~14k LOC)       │
│  swe2d/workbench/extracted/ (5 modules, ~8.5k LOC)       │
...
│  │_and_     │ │(drainage,│ │  (CRS      │ │(non-GUI   │ │
│  │forcing/  │ │ struct.) │ │   conv)    │ │  utils)   │ │
│  └──────────┘ └──────────┘ └────────────┘ └───────────┘ │
├──────────────────────────────────────────────────────────┤
...
swe2d/units.py  (standalone — CRS-derived unit conversion)
   ...
   └──→ swe2d/workbench/extracted/topology_and_io_methods.py
...
swe2d/workbench/
   ├── non_gui_qgis.py  (QGIS utility functions)
   ├── non_gui_runtime.py  (env helpers, parsing)
   ├── project_settings.py  (QGIS settings)
   └── extracted/ (5 lazy-loaded modules → swe2d_workbench_qt monolith)
```
...
| **Seam decomposition** | The original ~14k LOC monolith `swe2d_workbench_qt.py` is being incrementally extracted into `swe2d/workbench/extracted/` and `swe2d/runtime/` seam classes. |

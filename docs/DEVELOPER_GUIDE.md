...
A pre-built knowledge graph of the entire codebase is available at
`graphify-out/`.  It was generated by
[graphify](https://github.com/safishamsi/graphify) via AST extraction on
all Python/C++ files.  Use it to explore dependencies, find related modules,
and discover undocumented connections.

### 9.1 Graph Outputs

| File | Purpose |
|------|---------|
| `graphify-out/graph.html` | Interactive visualizer — open in browser to browse node relationships |
| `graphify-out/GRAPH_REPORT.md` | Audit report with god nodes, community sizes, suggested questions |
| `graphify-out/wiki/index.md` | Agent-crawlable wiki — one article per community (549 articles) |
| `graphify-out/graph.json` | Raw knowledge graph for GraphRAG or custom tooling |

### 9.2 Top Communities

These are the largest module clusters detected by community detection —
they reflect the actual coupling structure of the codebase:

| Community | Nodes | Core Files |
|-----------|-------|------------|
| GPU Solver Core | 165 | `cpp/src/swe2d_gpu.cu`, `swe2d_gpu.cuh` |
| Studio Dialog | 139 | `swe2d/workbench/studio_dialog.py` |
| Runtime Pipeline | 114 | `run_orchestrator.py`, `run_finalizer.py`, `runtime_step_executor.py` |
| Controller Protocol | 106 | `swe2d/workbench/controllers/` |
| Drainage & Pipes | 101 | `swe2d/extensions/drainage_network.py`, `extension_models.py` |
| Mesh & Boundaries | 92 | `swe2d/mesh/meshing.py`, `boundary_and_forcing/` |
| Hydraulic Structures | 81 | `swe2d/extensions/structures.py` |
| Mesh Generation | 73 | `swe2d/mesh/meshing.py` |
| Workbench Services | 68 | `swe2d/workbench/services/` |
| Tests | 83 | `tests/test_model_tab_view.py` |

### 9.3 God Nodes (Most Connected Abstractions)

These are the load-bearing classes/modules with the highest connection
count — good starting points for understanding the architecture:

| Node | Connections | Location |
|------|-------------|----------|
| `SWE2DWorkbenchStudioDialog` | 278 | `swe2d/workbench/studio_dialog.py` |
| `SWE2DDeviceState` | 167 | `cpp/src/swe2d_gpu.cuh` |
| `SWE2DResultsData` | 166 | `swe2d/results/data.py` |
| `SWE2DBackend` | 113 | `swe2d/runtime/backend.py` |
| `TestModelTabView` | 95 | `tests/test_model_tab_view.py` |
| `ModelTabView` | 85 | `swe2d/workbench/views/model_tab_view.py` |
| `TopologyTabView` | 76 | `swe2d/workbench/views/topology_tab_view.py` |
| `PYBIND11_MODULE()` | 61 | `cpp/src/swe2d_bindings.cpp` |
| `HydraulicStructureConfig` | 61 | `swe2d/extensions/extension_models.py` |
| `OverlayController` | 59 | `swe2d/workbench/controllers/overlay_controller.py` |

### 9.4 Queries

With the graph built, you can explore specific questions:

```bash
# Trace connections between modules
graphify query "How does the coupling controller connect to the GPU solver?"
...
- **[User Guide](USER_GUIDE.md)** — End-to-end user workflow
- **[GPU Architecture Report](SWE2D_GPU_ARCHITECTURE_REPORT.md)** — Device-side solver details

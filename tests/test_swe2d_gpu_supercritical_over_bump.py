"""
GPU supercritical flow over a bump (steady state).
...
Physical setup
--------------
Steady supercritical flow (q = 10.0 m²/s) passing over a smooth bump in a
25 m channel. Bump: z(x) = max(0, 0.2 − 0.05·(x−10)²). Downstream stage
fixed at 0.5 m.  The flow is entirely supercritical (Fr > 1); the
downstream stage BC is purely a numerical reference.  Simulation runs to
t = 10 s.
...
steady state, compare depth along the centreline against the ANUGA
analytical solution.

Tolerance: L1 error < 2 % of reference stage (0.5 m) → 0.010 m.
"""
...
_analytical = import_anuga_module(
    ...
    "supercritical_over_bump/analytical_supercritical.py"
)
...
class TestGPUSupercriticalOverBump(unittest.TestCase):
    anuga_reference = (
        "reference/anuga_validation_tests/analytical_exact/supercritical_over_bump/"
    )
    NX = 250
    NY = 3
    LX = 25.0
    ...
    def test_l1_error_vs_anuga(self):
        ...
        self.assertLess(
            l1,
            limit,
            msg=f"Supercritical over-bump L1 error {l1:.6f} m exceeds limit ({limit:.4f} m)",
        )

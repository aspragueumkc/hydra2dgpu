"""
GPU numerical-only validation — run-up on a linear beach.

Reference: reference/anuga_validation_tests/analytical_exact/runup_on_beach/

No analytical solution exists for this setup. The test checks that the solver
runs stably and that water motion occurs on a linear sloping beach.

Physical setup
--------------
1 m × 0.03 m channel. Linear bed slope z(x) = −x/2 (descending from 0 to −0.5).
Initial stage = −0.45 m everywhere, so only a thin wet strip exists at the
right (deep) end. Boundary conditions:
  - left  (x = 0):   WALL (reflective)
  - right (x = Lx):  STAGE = −0.1 (constant stage forcing)
  - top / bottom:     WALL (reflective)

No friction. T_END = 5.0 s — short enough for a fast test but long enough for
the stage forcing to propagate into the domain.

Checks
------
1. GPU active, all depths finite and non-negative (stability).
2. Water surface moves: standard deviation of h at end > 0 (non-trivial flow).
"""
...
class TestGPURunupOnBeach(unittest.TestCase):
    anuga_reference = (
        "reference/anuga_validation_tests/analytical_exact/runup_on_beach/"
    )
    NX = 100
    NY = 3
    LX = 1.0
    ...
    def _build(self, spatial_scheme: int = 0):
        ...
        # Bed: z(x) = -x/2  (linear slope descending from 0 to -0.5)
        def bed_func(x, y):
            return -x / 2.0
        ...
        # Initial stage = -0.45 m → depth capped at zero
        initial_stage = -0.45

...
class TestCompoundChannelFlow(unittest.TestCase):
    ...
    def test_gpu_stability(self):
        ...
        self.assertTrue(
            ...
            "GPU solver became inactive during compound-channel run",
        )
...
class TestCompoundChannelCPU(unittest.TestCase):
    ...
    Verifies positivity of depths and mass conservation.
    """
    ...
    def test_positivity_and_mass_cpu(self):
        mod = _load_module()

        H_ana = solve_stage(self.Q_TOTAL, self.S0,
                            self.W_MAIN, self.W_FP_EACH, self.H_BANK,
                            self.N_MAIN, self.N_FP)
        ana = {}
        ks  = compound_conveyance(H_ana, self.W_MAIN, self.W_FP_EACH,
                                   self.H_BANK, self.N_MAIN, self.N_FP)
        sqrt_S0 = np.sqrt(self.S0)
        ana["hu_main"] = ks["K_main"] * sqrt_S0 / self.W_MAIN
        ana["hu_fp"]   = ks["K_fp"]   * sqrt_S0 / (2.0 * self.W_FP_EACH)
        ana["H"]       = H_ana
        ...
        bc_n0, bc_n1, bc_tp, bc_vl = _compound_bc_arrays(
            self.NX, self.NY, self.LY,
            self.Y_LEFT_BANK, self.Y_RIGHT_BANK,
            q_main=ana["hu_main"], q_fp=ana["hu_fp"])
        ...
        z_bed_cell = bed(cx, cy)
        wse_cell   = self.S0 * (self.LX - cx) + H_ana
        h0  = np.maximum(0.0, wse_cell - z_bed_cell).astype(np.float64)
        hu0 = np.where(
            (cy >= self.Y_LEFT_BANK) & (cy <= self.Y_RIGHT_BANK),
            ana["hu_main"], ana["hu_fp"],
        ).astype(np.float64)
        hv0 = np.zeros(n_cells, dtype=np.float64)

        initial_vol = float(h0.sum())
        ...
        self.assertTrue(np.isfinite(h).all(),  "Non-finite h")
        self.assertTrue(np.isfinite(hu).all(), "Non-finite hu")
        self.assertGreaterEqual(float(h.min()), 0.0,
                                f"Negative depth {h.min():.6f}")

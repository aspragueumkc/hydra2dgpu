...
def install_qgis_mocks():
    ...
    # Ensure QApplication has instance() for hydra_plugin._harden_qt_quit_behavior
    class _HydraMockQApp:
        _inst = None
        @classmethod
        def instance(cls):
            if cls._inst is None:
                i = MagicMock(name="QApplication")
                i.quitOnLastWindowClosed = MagicMock(return_value=True)
                cls._inst = i
            return cls._inst
    if "qgis.PyQt.QtWidgets" in sys.modules:
        sys.modules["qgis.PyQt.QtWidgets"].QApplication = _HydraMockQApp
    if "PyQt5.QtWidgets" in sys.modules:
        sys.modules["PyQt5.QtWidgets"].QApplication = _HydraMockQApp
    ...
    # swe2d.results.high_perf_viewer (standalone in repo root, uses qgis.PyQt)
    if "swe2d.results.high_perf_viewer" not in sys.modules:
        m = types.ModuleType("swe2d.results.high_perf_viewer")
        m.render_unstructured_snapshot_image = MagicMock(
            return_value=bytearray(1024))
        sys.modules["swe2d.results.high_perf_viewer"] = m
    ...
    try:
        ...
        for _mod in ("PyQt5", "PyQt5.QtCore", "PyQt5.QtWidgets", "PyQt5.QtGui", "PyQt5.uic"):
            if _mod not in sys.modules:
                ...
                def _make_action(*a, **kw):
                    act = MagicMock(name="QAction")
                    act._obj_name = ""
                    act._text = str(a[0]) if a else ""
                    def _set_obj_name(n):
                        act._obj_name = str(n)
                    def _get_obj_name():
                        return act._obj_name
                    def _get_text():
                        return act._text
                    act.setObjectName = _set_obj_name
                    act.objectName = _get_obj_name
                    act.text = _get_text
                    return act
                class _MockQApplication:
                    ...
                    def instance(cls):
                        if cls._instance is None:
                            inst = MagicMock(name="QApplication")
                            inst.quitOnLastWindowClosed = MagicMock(return_value=True)
                            cls._instance = inst
                        return cls._instance
                m.QApplication = _MockQApplication
                m.QAction = _make_action
                def _make_menu(*args, **kwargs):
                    mm = MagicMock(name="QMenu")
                    mm._obj_name = ""
                    mm._actions = []
                    def _set_obj_name(n):
                        mm._obj_name = str(n)
                    def _get_obj_name():
                        return mm._obj_name
                    mm.setObjectName = _set_obj_name
                    mm.objectName = _get_obj_name
                    def _add_action(*a, **kw):
                        if a and isinstance(a[0], MagicMock):
                            mm._actions.append(a[0])
                            return a[0]
                        act = _make_action()
                        act.text.return_value = a[0] if a else ""
                        act.setObjectName(str(kw.get("objectName", "")))
                        mm._actions.append(act)
                        return act
                    def _actions():
                        return list(mm._actions)
                    def _remove_action(action):
                        try:
                            mm._actions.remove(action)
                        except ValueError:
                            pass
                    mm.addAction = _add_action
                    mm.actions = _actions
                    mm.removeAction = _remove_action
                    return mm
                m.QMenu = _make_menu
                ...
                m.QFrame = MagicMock
                m.QDialogButtonBox = MagicMock
                m.QFormLayout = MagicMock
                m.QProgressBar = MagicMock
                m.QTextEdit = MagicMock
                m.QCommandLinkButton = MagicMock
                sys.modules[_mod] = m

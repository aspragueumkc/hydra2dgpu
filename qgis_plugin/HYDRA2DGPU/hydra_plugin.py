...
class HydraQgisPlugin:
    def __init__(self, iface):
        ...
        # Phase 1.1: plugin moved to qgis_plugin/HYDRA2DGPU/, so the repo root
        # is two levels above plugin_dir (file -> HYDRA2DGPU -> qgis_plugin ->
        # repo). realpath() resolves the symlink under qgis prefix so paths
        # are stable. Three dirnames matches the rest of this file.
    ...
    def initGui(self):
        ...
        # Load CUDA DLL path from QSettings and set environment variable for CLI
        if sys.platform == "win32":
            try:
                s = QSettings("HYDRA2DGPU", "HYDRA2DGPU")
                cuda_dll_path = s.value("cuda_dll_path", "")
                if cuda_dll_path:
                    os.environ["HYDRA_CUDA_DLL_PATH"] = str(cuda_dll_path)
            except Exception as e:
                logging.getLogger(__name__).warning(
                    "Failed to load CUDA DLL path from QSettings: %s", e
                )
        ...
        # Auto-start the MCP bridge when the user opts in via HYDRA_MCP_BRIDGE=1.
        try:
            if str(os.environ.get("HYDRA_MCP_BRIDGE", "")).strip().lower() in (
                "1", "true", "yes", "on"
            ):
                QtCore.QTimer.singleShot(0, self._autostart_mcp_bridge)
        except Exception as _mcp_exc:
            logging.getLogger(__name__).debug(
                "Could not schedule MCP bridge autostart: %s", _mcp_exc
            )

    def _maybe_start_mcp_bridge(self) -> None:
        """Start the MCP bridge if ``HYDRA_MCP_BRIDGE=1`` is set.

        This entry point is retained for the Phase 4 unit test that asserts
        the bridge is created when the env var is enabled.
        """
        if str(os.environ.get("HYDRA_MCP_BRIDGE", "")).strip().lower() not in (
            "1", "true", "yes", "on"
        ):
            return
        self._autostart_mcp_bridge()

    def _autostart_mcp_bridge(self) -> None:
        """Start the HYDRA MCP bridge if it is not already running."""
        try:
            from tools.hydra_mcp.qgis_bridge import bootstrap_bridge_if_needed
            bridge = bootstrap_bridge_if_needed()
            if bridge is not None:
                self._mcp_bridge = bridge
                logging.getLogger(__name__).info(
                    "HYDRA_MCP_BRIDGE autostarted: socket=%s token_path=%s",
                    bridge.socket_name, bridge.token_path,
                )
        except Exception as exc:
            logging.getLogger(__name__).warning(
                "HYDRA_MCP_BRIDGE autostart failed: %s", exc
            )
            self._mcp_bridge = None
    ...
    def _show_install_dialog(self) -> None:
        """Show the first-launch installer; blocks until user accepts/cancels."""
        # Lazy import — installer.py lives alongside this module and is only
        # needed on the first-launch / missing-backend path. Avoids pulling
        # Qt in at plugin __init__ time.
        from .installer import BackendInstaller, InstallDialog  # noqa: E402
        installer = BackendInstaller(plugin_dir=self.plugin_dir)
        dlg = InstallDialog(installer, parent=self.iface.mainWindow())
        dlg.exec_()
...
class HYDRASettingsDialog(QDialog):
    ...
    def _check_and_install_deps(self):
        ...
        # Locate check_deps.py relative to this plugin (Phase 1.1: two levels up)

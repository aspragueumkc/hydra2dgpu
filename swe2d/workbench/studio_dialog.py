...
class SWE2DWorkbenchStudioDialog(QtWidgets.QDialog):
    ...
    def _build_component(
        ...
        name: str,
        title: str,
        area: QtCore.Qt.DockWidgetArea = QtCore.Qt.RightDockWidgetArea,
        tab_with: Optional[str] = None,
        populate: Optional[Callable[[QtWidgets.QDockWidget], None]] = None,
        ...
        if iface is not None and hasattr(iface, "addDockWidget"):
            try:
                ...
                logger_wb.warning("[ERROR] iface.addDockWidget failed for '%s': %s", name, e)
            try:
                dock.setFloating(False)
                dock.show()
            except Exception as e:
                logger_wb.warning("[ERROR] dock show failed for '%s': %s", name, e)
        return comp

    def _register_component(self, component: StudioComponent) -> None:
        """Register a dock component for automated host-window extraction.

        After registration, the component's dock is directly attached
        to the QGIS host window via ``iface.addDockWidget``.

        Parameters
        ----------
        component : StudioComponent
            The component descriptor with name, dock, area, etc.

        Example
        -------
        >>> self._register_component(StudioComponent(
        ...     name="results",
        ...     dock=self._state.studio_results_dock,
        ...     area=Qt.RightDockWidgetArea,
        ...     tab_with="inspector",
        ... ))
        """
        existing = self._state.studio_components.get(component.name)
        if existing is not None:
            self._log(f"[Studio] Overwriting registered component '{component.name}'")
        self._state.studio_components[component.name] = component
    def _studio_sync_view_mode(self, idx: int) -> None:
        """Sync view mode from combo to viewer (stub, no-op)."""
        pass  # No view mode combo in the new tab-based viewer

    def _sync_view_mode_to_studio(self, idx: int) -> None:
        """Sync view mode to studio from combo (stub, no-op)."""
        pass  # No view mode combo in the new tab-based viewer
        # and handles stack switching + results panel visibility.
    ...
    def _resolve_widget_attr(self, attr: str) -> Any:
    ...
    def _populate_inspector_dock(self, dock):
        ...
        # Model Settings tab — reads current values from dialog widgets
        model_page = QtWidgets.QWidget()
        model_layout = QtWidgets.QVBoxLayout(model_page)
        model_layout.setContentsMargins(6, 6, 6, 6)

        self._settings_tree = QtWidgets.QTreeWidget()
        self._settings_tree.setHeaderLabels(["Parameter", "Value"])
        self._settings_tree.setAlternatingRowColors(True)
        self._settings_tree.setAnimated(True)

        # Populate from available dialog widgets
        _groups = {
            "Solver": ["temporal_order_combo", "spatial_scheme_combo", "equation_set_combo",
                       "bed_friction_model_combo", "turbulence_model_combo"],
            "Time Stepping": ["cfl_spin", "dt_max_spin", "dt_fixed_spin",
                              "dt_initial_spin"],
            ...
            for key in keys:
                ...
                if w is not None:
                    try:
                        ...
                        logger_wb.warning("Exception reading widget value", exc_info=True)
                        val = "\u2014"
                        lbl = key
                    grp_item.addChild(QtWidgets.QTreeWidgetItem([str(lbl).replace("&", ""), str(val)]))
                else:
                    grp_item.addChild(QtWidgets.QTreeWidgetItem([key, "\u2014"]))
            self._settings_tree.addTopLevelItem(grp_item)
        ...
        for key in ("output_interval_edit", "run_duration_edit",
            ...
            if w is not None:
                try:
                    ...
                    val = "\u2014"
                    lbl = key
                run_item.addChild(QtWidgets.QTreeWidgetItem([str(lbl).replace("&", ""), str(val)]))
            else:
                run_item.addChild(QtWidgets.QTreeWidgetItem([key, "\u2014"]))
        self._settings_tree.addTopLevelItem(run_item)

        model_layout.addWidget(self._settings_tree)
        inspector_tabs.addTab(model_page, "Model Settings")

        # Mesh / Topology Settings tab
        mesh_page = QtWidgets.QWidget()
        mesh_layout = QtWidgets.QVBoxLayout(mesh_page)
        mesh_layout.setContentsMargins(6, 6, 6, 6)

        self._mesh_settings_tree = QtWidgets.QTreeWidget()
        self._mesh_settings_tree.setHeaderLabels(["Parameter", "Value"])
        self._mesh_settings_tree.setAlternatingRowColors(True)
        self._mesh_settings_tree.setAnimated(True)

        _mesh_groups: Dict[str, List[str]] = {
            "Topology Layers": [
            ...
            for key in keys:
                ...
                if w is not None:
                    try:
                        ...
                        val = "\u2014"
                        lbl = key
                    grp_item.addChild(QtWidgets.QTreeWidgetItem([str(lbl).replace("&", ""), val]))
                else:
                    grp_item.addChild(QtWidgets.QTreeWidgetItem([key, "\u2014"]))
            self._mesh_settings_tree.addTopLevelItem(grp_item)

        mesh_layout.addWidget(self._mesh_settings_tree)
        inspector_tabs.addTab(mesh_page, "Mesh Settings")

        # Help tab — cross-document search hub
        from swe2d.workbench.views.doc_viewer import DocHubWidget
        inspector_tabs.addTab(DocHubWidget(parent=self), "Help")

        dock.setWidget(inspector_tabs)

    def _on_results_play_state_changed(self, playing: bool) -> None:
        """Forward play-state changes to the temporal dock."""
        temporal = getattr(self, "_temporal_dock", None)
        if temporal is not None:
            temporal.on_play_state_changed(playing)
    ...
    def _build_ui(self):
        ...
        # View/theme combos (kept for signal wiring, placed as small bar)
        ...
        self._state.studio_view_mode_combo = QtWidgets.QComboBox()
        self._state.studio_view_mode_combo.addItems([
            "Mesh", "Depth", "Velocity magnitude",
            "Time-Series", "Profile", "Structures", "Network",
        ])
        view_bar_layout.addWidget(self._state.studio_view_mode_combo)
        view_bar_layout.addWidget(QtWidgets.QLabel(" Theme: "))
        self._state.studio_theme_combo = QtWidgets.QComboBox()
        self._state.studio_theme_combo.addItems(["Default", "Diagnostics", "Presentation"])
        view_bar_layout.addWidget(self._state.studio_theme_combo)
        view_bar_layout.addStretch(1)
        root.addWidget(view_bar)

        self._studio_viewer = SWE2DStudioViewer()

        if self.iface is None:
            root.addWidget(self._studio_viewer, stretch=1)
        ...
        self._build_component(
            ...
            tab_with="inspector",
            populate=lambda dock: dock.setWidget(self._studio_viewer),
            iface=self.iface,
        )
        ...
        self._state.studio_left_dock = self._state.studio_components["setup"].dock

        # Plot docks are created lazily — see _toggle_plot_dock
        self._ts_dock = None
        self._prof_dock = None
        self._struct_dock = None
        self._network_dock = None
        ...
        self._state.studio_inspector_dock = self._state.studio_components["inspector"].dock
        ...
        self._results_toolbox.run_refresh_requested.connect(
            self._on_results_refresh)
        self._results_toolbox.run_add_requested.connect(
            self._on_results_add)
        self._results_toolbox.run_remove_requested.connect(
            self._on_results_remove)
        self._results_toolbox.run_show_all.connect(
            self._on_results_show_all)
        self._results_toolbox.run_hide_all.connect(
            self._on_results_hide_all)
        # Line / profile signals
        self._results_toolbox.line_selected.connect(
            self._on_results_line_selected)
        self._results_toolbox.ts_var_changed.connect(
            self._on_results_ts_var_changed)
        self._results_toolbox.prof_var_changed.connect(
            self._on_results_prof_var_changed)
        self._results_toolbox.profile_options_changed.connect(
            self._on_results_profile_options_changed)
        # Coupling page signals
        self._results_toolbox.coupling_metric_changed.connect(
            self._on_coupling_metric_changed)
        self._results_toolbox.coupling_element_changed.connect(
            self._on_coupling_element_changed)
        self._build_component(
            name="results",
            title="HYDRA2D Results",
            area=QtCore.Qt.RightDockWidgetArea,
            tab_with="inspector",
            populate=lambda dock: dock.setWidget(self._results_toolbox),
            iface=self.iface,
        )
        self._state.studio_results_dock = self._state.studio_components["results"].dock

        # -- Temporal dock: animation bar
        self._temporal_dock = TemporalDockWidget()
        self._build_component(
            name="temporal",
            title="HYDRA2D Temporal",
            area=QtCore.Qt.BottomDockWidgetArea,
            populate=lambda dock: dock.setWidget(self._temporal_dock),
            iface=self.iface,
        )

        # -- Log dock: standalone bottom panel --
        self._build_component(
            name="log",
            title="HYDRA2D Log",
            area=QtCore.Qt.BottomDockWidgetArea,
        ...
        footer = QtWidgets.QStatusBar(self)
        self._state.studio_status_label = QtWidgets.QLabel("")
        footer.addPermanentWidget(self._state.studio_status_label, 1)
        root.addWidget(footer)
        self._studio_update_status()

        if hasattr(self, "view_mode_combo") and self.view_mode_combo is not None:
            try:
                self._state.studio_view_mode_combo.setCurrentIndex(max(0, int(self.view_mode_combo.currentIndex())))
            except Exception as e:
                self._log(f"[ERROR] build ui failed: {e}")
                pass
        ...
        self._state.studio_view_mode_combo.currentIndexChanged.connect(self._studio_sync_view_mode)
        self._state.studio_theme_combo.currentTextChanged.connect(self._studio_apply_visual_profile)
    ...
    def _find_widget(self, wtype, name):
        ...
        if w is None and self.iface is not None:
            try:

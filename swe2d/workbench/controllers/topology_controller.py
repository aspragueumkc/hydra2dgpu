...
class TopologyController:
    ...
    def _populate_gmsh_quality_controls(self) -> None:
        ...
        Delegates widget construction to the view, then wires signals from here.
        """
        view = self._view
        topo = view._topology_tab_view
        if topo is None:
            return
        widgets = topo._populate_gmsh_quality_controls()
        self._wire_topology_tab_controls(widgets, topo.update_control_summary)
    ...
    def _wire_topology_tab_controls(
        ...
        """Connect topology tab widget signals to the summary updater."""
        if not widgets:
            return
        ...
        def _w(sig, handler):
            ...
            try:
                ...
                try:
                    self._view._log(f"[ERROR] RuntimeError in topology_controller.py: {_e}")
                except Exception:
                    pass

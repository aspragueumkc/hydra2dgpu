...
def _qml_file_path(layer_name: str) -> str | None:
    ...
    Candidates, in order:
    1. repo-root ``QML/`` (dev layout — four levels up from this module)
    2. ``<plugin-dir>/QML/`` for every sys.path entry (production layout —
       ``tools/package_plugin.py`` bundles ``QML/`` inside the zip)
    """
    qml_dirs = [
        os.path.join(
    ...
    for entry in sys.path:
        candidate = os.path.join(entry, "HYDRA2DGPU", "QML")
        if candidate not in qml_dirs:

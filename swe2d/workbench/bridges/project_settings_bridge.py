...
def read_project_entry_text(
    *,
    ...
    """Read a string entry from QGIS project settings with fallback default."""
...
def load_project_json(
    *,
    ...
    """Load and parse a JSON value from QGIS project settings."""
...
def write_project_json(
    *,
    ...
    """Serialize and write a JSON value to QGIS project settings."""
    if not have_qgis_core or qgs_project_cls is None:
        return False
    try:
...
def build_layer_selector_state(specs: Sequence[tuple[str, object]]) -> Dict[str, object]:
    """Build a serialisable dict of current layer combo selections for persistence."""
...
def parse_layer_selector_state(payload: object) -> Dict[str, Dict[str, str]]:
    """Parse saved layer selector state back into a structured dict."""
...
def collect_workbench_widget_state(*, ui: object, widget_attrs: Sequence[str], qtwidgets_module: object) -> Dict[str, object]:
    """Collect current values from a list of named widgets into a serialisable dict."""
    ...
    for attr_name in widget_attrs:
        ...
        # Skip deleted C++ objects (e.g. widgets from removed tabs/panels)
        try:
            _ = widget.objectName()
        except RuntimeError:
            continue
...
def restore_workbench_widget_state(
    *,
    ...
    """Restore saved widget values onto a UI, returning the count of restored widgets."""
    ...
    for attr_name, widget_info in widgets_data.items():
        ...
        # Skip deleted C++ QObjects (e.g. widgets from removed tabs)
        try:
            _ = widget.objectName()
        except RuntimeError:
            if callable(log_callback):
                log_callback(f"[DEBUG] restore: skipping deleted widget '{attr_name}'")
            continue
        ...
        try:
            if isinstance(widget, spin_box_cls):
                ...
                for index in range(widget.count()):
                    ...
                    # JSON serialises tuples → lists, so normalise for comparison
                    if isinstance(value, list) and isinstance(data, tuple):
                        ...
                        break
                if not found:
                    # Fallback: try index if value is numeric; skip otherwise
                    try:
                        widget.setCurrentIndex(int(value))
                    except (TypeError, ValueError):
                        pass  # e.g. tuple/list value that didn't match any itemData
                restored_count += 1
            elif isinstance(widget, check_box_cls):

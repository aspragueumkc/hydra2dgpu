"""QGIS map tool for interactive polyline drawing.
...
if _HAVE_QGIS_MAP_TOOL:
    ...
    class SWE2DLineDrawTool(QgsMapTool):
        ...
        def canvasPressEvent(self, event):
            """Handle mouse press: left-click adds a point, right-click finishes the line."""
        ...
        def canvasMoveEvent(self, event):
            """Update the rubberband preview line as the mouse moves over the canvas."""
            if not self._points:
                return
            pt = self.toMapCoordinates(event.pos())
        ...
        def canvasDoubleClickEvent(self, event):
            """Add a point on double-click and finish the line."""
            pt = self.toMapCoordinates(event.pos())
            self._points.append(QgsPointXY(pt.x(), pt.y()))
            self._finish_line()

        def keyPressEvent(self, event):
            """Cancel the current line on Escape key press."""
            if event.key() == Qt.Key_Escape:
                self._points = []
                self._rb.reset(QgsWkbTypes.GeometryType.LineGeometry)

        def deactivate(self):
            """Clean up the rubberband when the tool is deactivated."""
            self._rb.reset(QgsWkbTypes.GeometryType.LineGeometry)
            super().deactivate()

        def _redraw_rubberband(self, moving_pt=None):
            """Redraw the rubberband line from collected points, optionally including a preview point."""
            self._rb.reset(QgsWkbTypes.GeometryType.LineGeometry)
        ...
        def _finish_line(self):
            """Emit the finished polyline geometry and reset the tool state."""

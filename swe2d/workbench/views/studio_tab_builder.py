...
def wire_map_tab_action_signals(dialog) -> None:
    ...
    handlers = {
        ...
        "mesh_gpkg_load_combo": (v.mesh_gpkg_load_combo, None),
...
def show_tab_detach_menu(dialog, tab_widget, pos):
    """Show a context menu to detach the tab at the given position."""
    ...
    if chosen == detach_action:
        detach_tab(dialog, tab_widget, idx)


def detach_tab(dialog, tab_widget, index):
    """Detach a tab into a separate floating dialog window."""
    ...
    try:
        ...
        dialog._log(f"[ERROR] detach tab failed: {e}")
        return
    ...
    dialog._detached_panel_dialogs.append(dlg)
    dlg.finished.connect(lambda: dialog._detached_panel_dialogs.remove(dlg) if dlg in dialog._detached_panel_dialogs else None)
    dlg.resize(600, 500)
    dlg.show()
...
def format_elapsed(dialog, seconds: float) -> str:

...
class SWE2DRunFinalizer:
    ...
    def _flow_unit_label(self) -> str:
        ...
        unit = str(getattr(self._ui, "_length_unit_name", "m")).strip().lower()
        if unit == "ft":
            return "cfs"
        if unit == "m":
            return "cms"
        ...
        *,
        ...
        if gpkg_results_path and hasattr(self._ui, "_persist_conservation_forensics_to_geopackage"):
            try:
                ...
                self._ui._log(f"  conservation persisted to {gpkg_results_path} in {(time.perf_counter() - _t0) * 1000:.0f} ms")
            except Exception as exc:
                self._ui._log(f"Conservation forensic persistence warning: {exc}")

        _t0 = time.perf_counter()
        if gpkg_results_path and save_line_results and self._ui._line_snapshot_rows:
            persist_line_results_to_geopackage(
                gpkg_path=gpkg_results_path,
                run_id=run_id,
                rows=self._ui._line_snapshot_rows,
                mesh_interval_s=output_interval_s,
                line_interval_s=line_output_interval_s,
                profile_rows=self._ui._line_snapshot_profile_rows,
                log_fn=self._ui._log,
                results_table_name_fn=self._ui._results_table_name,
            )
            self._ui._log(f"  line results saved to {gpkg_results_path} in {(time.perf_counter() - _t0) * 1000:.0f} ms")

        _t0 = time.perf_counter()
        if gpkg_results_path and save_coupling_results and self._ui._coupling_snapshot_rows:
            ...
            self._ui._log(f"  coupling results saved to {gpkg_results_path} in {(time.perf_counter() - _t0) * 1000:.0f} ms")

        _t0 = time.perf_counter()
        if gpkg_results_path and save_mesh_results and self._ui._snapshot_timesteps:
            ...
            if mesh_rows:
                ...
                self._ui._log(f"  mesh results saved to {gpkg_results_path} in {(time.perf_counter() - _t0) * 1000:.0f} ms")
        ...
        try:
            ...
            self._ui._log(f"  overlay sync + update in {(time.perf_counter() - _t0) * 1000:.0f} ms")
        except Exception:
        ...
        if gpkg_results_path and save_run_log and run_id:
            ...
            self._ui._log(f"  run log saved to {gpkg_results_path} in {(time.perf_counter() - _t0) * 1000:.0f} ms")

        if gpkg_results_path:
            try:
                update_run_snapshot_tag(gpkg_results_path, run_id, is_snapshot=False,
                                        table_name_fn=self._ui._results_table_name)
            except Exception:
                pass

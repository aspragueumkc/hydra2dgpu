...
def nearest_cell_index(
    ...
    """Return index of cell whose centroid is nearest to (x, y).

    Pure geometry — no Qt, no widget access.  Used by View methods that
    need spatial lookup without importing numpy themselves.
    """
    dx = cell_cx - float(x)
    dy = cell_cy - float(y)

...
def _sample_coupling_object_metrics(cc, t_s: float, dt: float, _h_s, _results_data=None) -> list:
    ...
    if cfg is not None:
        cell_depth_arr = state.get("cell_depth")
        cell_depth_max = (
            float(np.max(cell_depth_arr))
            if cell_depth_arr is not None and cell_depth_arr.size > 0
            else 0.0
        )
        # Build node → cell index mapping for conceptual nodes.
        # Device cell layout: [pipe_0..pipe_N-1] [manhole_0..M-1] [inlet_0..K-1]
        # A "junction" node at position k among all junction nodes in cfg.nodes
        # maps to manhole cell at (n_pipe_cells + k).  An "inlet" node at
        # position k among all inlet nodes maps to inlet cell at
        # (n_pipe_cells + n_manhole_cells + k).
        cell_class_arr = state.get("cell_class")
        n_pipe = int(state.get("n_pipe_cells", 0))
        n_mh = int(state.get("n_manhole_cells", 0))
        n_inl = int(state.get("n_inlet_cells", 0))
        # Pre-scan: count how many junction and inlet nodes are in cfg.nodes
        # so we can compute per-node-type offsets.
        junction_indices: list[int] = []
        inlet_indices: list[int] = []
        for ni, nd in enumerate(getattr(cfg, "nodes", [])):
            nt = str(getattr(nd, "node_type", "junction")).strip().lower()
            if nt == "inlet":
                inlet_indices.append(ni)
            elif nt in ("junction", "storage", ""):
                junction_indices.append(ni)
        for i, node in enumerate(getattr(cfg, "nodes", [])):
            node_id = str(getattr(node, "node_id", str(i)))
            nt = str(getattr(node, "node_type", "junction")).strip().lower()
            if nt == "inlet" and n_inl > 0:
                # Inlet node → use its dedicated inlet storage cell
                idx_in_inlets = inlet_indices.index(i) if i in inlet_indices else -1
                if idx_in_inlets >= 0:
                    cell_ci = n_pipe + n_mh + idx_in_inlets
                    if cell_depth_arr is not None and cell_ci < len(cell_depth_arr):
                        depth = float(cell_depth_arr[cell_ci])
                    else:
                        depth = cell_depth_max
                else:
                    depth = cell_depth_max
            elif nt in ("junction", "storage", "") and n_mh > 0:
                # Junction / storage node → use its manhole storage cell
                idx_in_junctions = junction_indices.index(i) if i in junction_indices else -1
                if idx_in_junctions >= 0:
                    cell_ci = n_pipe + idx_in_junctions
                    if cell_depth_arr is not None and cell_ci < len(cell_depth_arr):
                        depth = float(cell_depth_arr[cell_ci])
                    else:
                        depth = cell_depth_max
        ...
        for li, link in enumerate(getattr(cfg, "links", [])):
            ...
            if li < len(state["link_q"]):
                flow = float(state["link_q"][li])
        ...
        # Drainage cell: per-pipe-cell Q, h, q, y (matches new API).
        cell_q = state.get("cell_q")
        cell_h = state.get("cell_h")
        cell_Q = state.get("cell_Q")
        cell_y = state.get("cell_y")
        cell_owner_link = state.get("cell_owner_link")
        cell_sub_idx = state.get("cell_sub_idx")  # directly from C++
        # Per-cell geometry for profile rendering
        cell_invert = state.get("cell_invert")
        cell_width = state.get("cell_width")
        cell_height = state.get("cell_height")
        cell_shape_type = state.get("cell_shape_type")

        if cell_q is not None and cell_owner_link is not None and cfg is not None:
            ...
            if cell_sub_idx is not None and len(cell_sub_idx) == n_cells:
                ...
                for c in range(n_cells):
                    ...
                    for metric, arr in [("velocity", cell_q), ("depth", cell_h),
                                       ("flow", cell_Q), ("wse", cell_y)]:
                        rows.append({
                            "t_s": t_s,
                            ...
                            "value": float(arr[c]) if arr is not None else 0.0,
                            ...
                            "cell_height": float(cell_height[c]) if cell_height is not None and c < len(cell_height) else 1.0,
                            "cell_shape_type": int(cell_shape_type[c]) if cell_shape_type is not None and c < len(cell_shape_type) else 0,
                        })
            else:
                ...
                for c in range(n_cells):
                    ...
                    link = links[li]
                    link_id = str(getattr(link, "link_id", str(li)))
                    for metric, arr in [("velocity", cell_q), ("depth", cell_h), ("flow", cell_Q), ("wse", cell_y)]:
                        rows.append({
                            "t_s": t_s,
                            "component": "drainage_cell",
                            "metric": metric,
    ...
    if hasattr(cc, "_structures_cfg") and cc._structures_cfg:
        nb_flows = state["struct_q"] if state["struct_q"].size > 0 else getattr(cc, "_last_structure_flows", None)
...
def build_pipe_cell_keys(cc) -> List[Tuple[str, int, str]]:
    ...
    mcl_arr = getattr(dsoa, "max_cell_length", None)
    links = getattr(cfg, "links", [])
    ...
    for li, link in enumerate(links):
        ...
        mcl = float(mcl_arr[li]) if mcl_arr is not None and li < len(mcl_arr) else 0.0
        n_sub = 1

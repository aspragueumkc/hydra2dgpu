...
"""Pipe-network service: node/link geometry, topology validation, and import."""
...
def _opt_float_list(value, fallback=None, log_fn=None):
    """Parse a comma-separated float string into a list[float].

    Returns ``fallback`` (default ``[]``) when the input is None / empty
    or contains no parseable values.  Whitespace is stripped from each
    token; tokens that fail float conversion are skipped (and logged via
    ``log_fn``).  Used for the outfall rating / tabular arrays stored as
    comma-separated strings in the GPKG schema (SPEC §2.8).
    """
    if fallback is None:
        fallback = []
    if value is None:
    ...
    for tok in sval.split(","):
        ...
        try:
            ...
            if log_fn is not None:
                log_fn(f"[ERROR] opt_float_list token {tok!r}: {e}")
            continue
    return out
...
def build_pipe_network_config(
    *,
    ...
    for ft in node_layer.getFeatures():
        ...
        # ── Pipe-end overflow gating (per-node, used when node_type=="pipe_end") ──
        # Reads from the GPKG layer's enable_overflow / overflow_elevation /
        # max_overflow_rate fields.  These were previously schema-declared but
        # never consumed by the Python loader (the audit "MISSING" category).
        enable_overflow = _opt_bool(
            ft["enable_overflow"] if "enable_overflow" in node_fields else None,
            False,
            log_fn=_log,
        )
        overflow_elevation = _opt_float(
            ft["overflow_elevation"] if "overflow_elevation" in node_fields else None,
            None,
            log_fn=_log,
        )
        max_overflow_rate = _opt_float(
            ft["max_overflow_rate"] if "max_overflow_rate" in node_fields else None,
            None,
            log_fn=_log,
        )

        # ── SPEC §2.8 — outfall BC fields ─────────────────────────────────
        # Mode defaults to "free" which matches the legacy FREE-only
        # behaviour.  fixed_wse and rating/tabular arrays are optional;
        # their string payloads are parsed into Python lists so they
        # flow into the DrainageNode dataclass unchanged.
        outfall_mode_str = str(
            ft["outfall_mode"]
            if "outfall_mode" in node_fields
            else "free"
        ).strip().lower() or "free"
        outfall_fixed_wse_val = _opt_float(
            ft["outfall_fixed_wse"] if "outfall_fixed_wse" in node_fields else None,
            0.0,
            log_fn=_log,
        )
        outfall_rating_wse = _opt_float_list(
            ft["outfall_rating_wse"] if "outfall_rating_wse" in node_fields else None,
            log_fn=_log,
        )
        outfall_rating_q = _opt_float_list(
            ft["outfall_rating_q"] if "outfall_rating_q" in node_fields else None,
            log_fn=_log,
        )
        outfall_rating_pairs = list(zip(outfall_rating_wse, outfall_rating_q)) if (
            outfall_rating_wse and outfall_rating_q
            and len(outfall_rating_wse) == len(outfall_rating_q)
        ) else []
        outfall_tabular_time = _opt_float_list(
            ft["outfall_tabular_time"] if "outfall_tabular_time" in node_fields else None,
            log_fn=_log,
        )
        outfall_tabular_wse = _opt_float_list(
            ft["outfall_tabular_wse"] if "outfall_tabular_wse" in node_fields else None,
            log_fn=_log,
        )
        # ── SPEC §2.10 — junction surcharge overflow fields ──────────────
        junction_overflow_diam = _opt_float(
            ft["junction_overflow_diam"] if "junction_overflow_diam" in node_fields else None,
            0.0,
            log_fn=_log,
        )
        junction_overflow_coeff = _opt_float(
            ft["junction_overflow_coeff"] if "junction_overflow_coeff" in node_fields else None,
            1.7,
            log_fn=_log,
        )
        junction_max_overflow_rate = _opt_float(
            ft["junction_max_overflow_rate"] if "junction_max_overflow_rate" in node_fields else None,
            0.0,
            log_fn=_log,
        )
        inlet_box_length = _opt_float(
            ft["inlet_box_length"] if "inlet_box_length" in node_fields else None,
            None,
            log_fn=_log,
        )
        inlet_box_width = _opt_float(
            ft["inlet_box_width"] if "inlet_box_width" in node_fields else None,
            None,
            log_fn=_log,
        )
        inlet_loss_k = _opt_float(
            ft["inlet_loss_k"] if "inlet_loss_k" in node_fields else None,
            None,
            log_fn=_log,
        )
        outlet_loss_k = _opt_float(
            ft["outlet_loss_k"] if "outlet_loss_k" in node_fields else None,
            None,
            log_fn=_log,
        )
        ...
        node = DrainageNode(
            ...
            rim_elev=float(rim),
            node_type=node_type,
            outfall_mode=outfall_mode_str,
            outfall_fixed_wse=float(outfall_fixed_wse_val),
            outfall_rating_table=outfall_rating_pairs,
            enable_overflow=enable_overflow,
            overflow_elevation=overflow_elevation,
            max_overflow_rate=max_overflow_rate,
            inlet_box_length=inlet_box_length,
            inlet_box_width=inlet_box_width,
            inlet_loss_k=inlet_loss_k,
            outlet_loss_k=outlet_loss_k,
            metadata={
                "surface_area": float(
                    ft["surface_area"]
                ...
                # Junction overflow params live in metadata so the
                # DrainageNode dataclass signature stays stable until
                # extension_models.py gains explicit junction fields.
                "junction_overflow_diam": float(junction_overflow_diam),
                "junction_overflow_coeff": float(junction_overflow_coeff),
                "junction_max_overflow_rate": float(junction_max_overflow_rate),
                # Tabular WSE series — packed as parallel lists of floats
                # so the packer / loader can rebuild the SoA arrays
                # without re-parsing the GPKG strings.
                "outfall_tabular_time": list(outfall_tabular_time),
                "outfall_tabular_wse": list(outfall_tabular_wse),
            },
        )
        nodes.append(node)
    ...
    for ft in link_layer.getFeatures():
        ...
        # ── Loss coefficient normalization ─────────────────────────────
        # Normalize FHWA naming (entrance_loss_k/exit_loss_k) to primary
        # field names (inlet_loss_k/outlet_loss_k).  The schema supports both
        # for backward compatibility with FHWA HDS-5 tools and legacy datasets.
        # Priority: explicit field > alias > default (Ke=0.5, Kx=1.0).
        ...
        max_cell_length_val = _opt_float(
            ft["max_cell_length"] if "max_cell_length" in link_fields else None,
            0.0,
            log_fn=_log,
        )
        ...
        links.append(
            DrainageLink(
                ...
                max_cell_length=max_cell_length_val,
        ...
        for nid in (str(lnk.from_node_id), str(lnk.to_node_id)):
            ...
            pipe_ends.append(
                PipeEndExchange(
                    ...
                    enable_overflow=bool(getattr(node, "enable_overflow", False)),
                    overflow_elevation=getattr(node, "overflow_elevation", None),
                    max_overflow_rate=getattr(node, "max_overflow_rate", None),
                )
            )
    ...
    friction_method = int(config.get("friction_method", 0))
    ...
    surcharge_method = int(config.get("surcharge_method", 0))

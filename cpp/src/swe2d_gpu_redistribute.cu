...
/** Ensure persistent redistribution device buffers have capacity for given sizes.
    Reallocates on capacity miss and resets data_hash to force re-upload. */
...
/** Compute content hash of static redistribution geometry.
    Used to skip host-to-device uploads when data is unchanged. */
...
/** Host-callable redistribution of structure/bridge source terms across a corridor.
    Reverses the single-cell injection already applied by the coupling source kernel,
    then distributes flow across corridor cells using pre-computed weights.
    Manages device memory allocation and content-hash tracking for static geometry. */
...
// ─────────────────────────────────────────────────────────────────────────────
/** Element-wise array scaling kernel. @global @param n Array length
    @param data [in/out] Array to scale @param factor Scale factor */

...
constexpr double BW2D_GRAVITY_FTPS2 = 32.2;
...
static py::array_t<double> compute_structure_flows_native(
    ...
    py::array_t<double, py::array::c_style | py::array::forcecast> culvert_area_m2,
    ...
    double gravity_mps2)
{
    ...
    require_array(culvert_area_m2, ns, "culvert_area_m2");
    ...
    constexpr double FT_PER_M = 3.280839895013123;
    constexpr double CFS_PER_CMS = 35.31466672148859;
    ...
    for (py::ssize_t i = 0; i < ns; ++i) {
        ...
        if (stype[i] == 3) {
            ...
            double q = bw2d_orifice_q(wu, wd, area, cd.data()[i], gravity_mps2);
            if (qmax >= 0.0) q = bw2d_clamp(q, -qmax, qmax);
            qout[i] = q;
            continue;
        }
        if (stype[i] == 4) {
            ...
            if (area > 0.0 && std::abs(dh) > 1.0e-12) {
                double q = area * std::sqrt(std::max(0.0, 2.0 * gravity_mps2 * std::abs(dh))) / loss_scale;
    ...
    double gravity_mps2)
{
    ...
    swe2d_gpu_compute_structure_flows(
        ...
        ns ? culvert_area_m2.data() : nullptr,
    ...
    m.def("swe2d_gpu_compute_structure_and_coupling_sources",
           ...
           -> py::array_t<double>
        {
            ...
            swe2d_gpu_compute_structure_and_coupling_sources(
                ...
                (n_structures > 0) ? culvert_area_m2.data() : nullptr,
...
#ifdef HYDRA_HAS_CUDA
    // ── Persistent GPU coupling: preload-once, run-on-device path ──
    // Global device pointer management.
    m.def("swe2d_gpu_set_coupling_device_global",
        [](uintptr_t dev_ptr) {
            swe2d_gpu_set_coupling_device_global(
                reinterpret_cast<SWE2DDeviceState*>(dev_ptr));
        },
        py::arg("dev_ptr"),
        "Set the global GPU device pointer used by persistent coupling functions.");

    // Upload static structure parameters to persistent device buffers.
    m.def("swe2d_gpu_preload_structure_params",
           ...
           double gravity_mps2 = 9.81)
        {
            ...
            swe2d_gpu_preload_structure_params(
                nullptr, n,
                n > 0 ? structure_type.data() : nullptr,
                n > 0 ? upstream_cell.data() : nullptr,
                n > 0 ? downstream_cell.data() : nullptr,
                n > 0 ? crest_elev.data() : nullptr,
                n > 0 ? width.data() : nullptr,
                n > 0 ? height.data() : nullptr,
                n > 0 ? diameter.data() : nullptr,
                n > 0 ? length.data() : nullptr,
                n > 0 ? roughness_n.data() : nullptr,
                n > 0 ? coeff.data() : nullptr,
                n > 0 ? cd.data() : nullptr,
                n > 0 ? opening.data() : nullptr,
                n > 0 ? q_pump.data() : nullptr,
                n > 0 ? max_flow.data() : nullptr,
                n > 0 ? culvert_code.data() : nullptr,
                n > 0 ? culvert_shape.data() : nullptr,
                n > 0 ? culvert_rise.data() : nullptr,
                n > 0 ? culvert_span.data() : nullptr,
                n > 0 ? culvert_area_m2.data() : nullptr,
                n > 0 ? culvert_barrels.data() : nullptr,
                n > 0 ? culvert_slope.data() : nullptr,
                n > 0 ? inlet_invert_elev.data() : nullptr,
                n > 0 ? outlet_invert_elev.data() : nullptr,
                n > 0 ? entrance_loss_k.data() : nullptr,
                n > 0 ? exit_loss_k.data() : nullptr,
                n > 0 ? embankment_enabled.data() : nullptr,
                n > 0 ? embankment_crest_elev.data() : nullptr,
                n > 0 ? embankment_overflow_width.data() : nullptr,
                n > 0 ? embankment_weir_coeff.data() : nullptr,
                gravity_mps2);
        },
        py::arg("structure_type"),
        py::arg("upstream_cell"),
        ...
        py::arg("embankment_weir_coeff"),
        py::arg("gravity_mps2") = 9.81,
        "Preload static structure parameters to persistent GPU buffers.");

    // Parameterless overloads using the global device pointer.
    m.def("swe2d_gpu_preload_structure_params_nogdev",
        [](py::array_t<int32_t, py::array::c_style | py::array::forcecast> structure_type,
           py::array_t<int32_t, py::array::c_style | py::array::forcecast> upstream_cell,
           py::array_t<int32_t, py::array::c_style | py::array::forcecast> downstream_cell,
           py::array_t<double, py::array::c_style | py::array::forcecast> crest_elev,
        ...
        "Preload structure params using global device pointer (no dev param needed).");

    m.def("swe2d_gpu_preload_coupling_cell_area_nogdev",
        [](py::array_t<double, py::array::c_style | py::array::forcecast> cell_area_m2)
        {
            swe2d_gpu_preload_coupling_cell_area(
                nullptr,
                static_cast<int32_t>(cell_area_m2.size()),
                cell_area_m2.data());
        },
        py::arg("cell_area_m2"),
        "Preload cell_area using global device pointer.");

    m.def("swe2d_gpu_compute_coupling_full_on_device_nogdev",
        [](py::array_t<double, py::array::c_style | py::array::forcecast> cell_wse,
           ...
           py::array_t<double, py::array::c_style | py::array::forcecast> inlet_flow_cms)
        {
            swe2d_gpu_compute_coupling_full_on_device(
                nullptr,
                static_cast<int32_t>(cell_wse.size()),
                n_structures,
                cell_wse.data(),
                static_cast<int32_t>(inlet_cell.size()),
                inlet_cell.size() > 0 ? inlet_cell.data() : nullptr,
        ...
        py::arg("cell_wse"),
        py::arg("n_structures") = 0,
        py::arg("inlet_cell") = py::array_t<int32_t>(),
        py::arg("inlet_flow_cms") = py::array_t<double>(),
        "Run on-device coupling using global device pointer.");

    // Upload cell_area to persistent GPU buffer (call once at startup).

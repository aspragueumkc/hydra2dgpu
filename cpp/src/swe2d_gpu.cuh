...
// ── Persistent GPU coupling path: upload-once, run-on-device ──
// Wire the GPU device state to the persistent coupling subsystem so that
// on-device coupling functions can operate without explicit dev parameters.
void swe2d_gpu_set_coupling_device_global(SWE2DDeviceState* dev);
SWE2DDeviceState* swe2d_gpu_get_coupling_device_global();

// Preload static structure parameters onto the device so later coupling
// calls only transfer cell_wse and return source rates.
// Must be called once before any on-device coupling calls.
// The dev->sf_ws buffers are allocated/reused automatically.
...
// Upload cell_area to persistent device workspace (call once at startup).
void swe2d_gpu_preload_coupling_cell_area(
    SWE2DDeviceState* dev,
    int32_t n_cells,
    const double* cell_area_m2);

// Run the full structure-flows + coupling-sources pipeline entirely on-device,
// writing per-cell source rates [m/s] directly into dev->d_external_source_mps.
// Only cell_wse is transferred from host; all structure params and cell_area
// must have been preloaded via the preload functions above.

...
/** GPU kernel: result[i] = a[i] - b[i], element-wise double subtraction.
 * 1 thread per cell.
 * @global */
__global__ __launch_bounds__(256, 4) void swe2d_double_sub_kernel(
    int32_t n,
...
/** GPU kernel: inout[i] *= scale, in-place scaling.
 * 1 thread per cell.
 * @global */
__global__ __launch_bounds__(256, 4) void swe2d_double_scale_inplace_kernel(
    int32_t n,
    double scale,
...
/** GPU kernel: result[i] = (double)state[i] - d[i], where state is State and d is double.
 * Used to compute k_i = h_i - y_i when y_i is already a double and h_i is a State.
 * 1 thread per cell.
 * @global */
__global__ __launch_bounds__(256, 4) void swe2d_state_subtract_double_kernel(
    int32_t n,
    ...
    double* __restrict__ result)
{
    int32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    result[i] = static_cast<double>(state[i]) - d[i];
}

/** GPU kernel: compute y0 + dt*(a1*k1 + a2*k2) into d_h (used in Stage 2).
 * Tighter than swe2d_rk5_stage34_prep_kernel when a3 = 0 (we save kernel arg overhead).
 * @global */
__global__ __launch_bounds__(256, 4) void swe2d_rk5_stage2_intermediate_kernel(
    int32_t n,
    double dt,
    const double* __restrict__ y0_h,
    const double* __restrict__ y0_hu,
    const double* __restrict__ y0_hv,
    const double* __restrict__ k1_h,
    const double* __restrict__ k1_hu,
    const double* __restrict__ k1_hv,
    State* __restrict__ h,
    State* __restrict__ hu,
    State* __restrict__ hv)
{
    int32_t c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= n) return;
    h[c]  = static_cast<State>(y0_h[c]  + dt * (1.0 / 5.0)  * k1_h[c]);
    hu[c] = static_cast<State>(y0_hu[c] + dt * (1.0 / 5.0)  * k1_hu[c]);
    hv[c] = static_cast<State>(y0_hv[c] + dt * (1.0 / 5.0)  * k1_hv[c]);
}

/** GPU kernel: RK4 Stage 2 prep — y0 + (dt/2)*k1 (textbook RK4).
 * Tighter than swe2d_rk5_stage34_prep_kernel when a3 = 0.
 * @global */
__global__ __launch_bounds__(256, 4) void swe2d_rk4_stage2_intermediate_kernel(
    int32_t n,
    double dt,
    const double* __restrict__ y0_h,
    const double* __restrict__ y0_hu,
    ...
    State* __restrict__ hv)
{
    int32_t c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= n) return;
    h[c]  = static_cast<State>(y0_h[c]  + 0.5 * dt * k1_h[c]);
    hu[c] = static_cast<State>(y0_hu[c] + 0.5 * dt * k1_hu[c]);
    hv[c] = static_cast<State>(y0_hv[c] + 0.5 * dt * k1_hv[c]);
}
...
/** GPU kernel: update rainfall source rate at a fixed interval.
 * 1 thread per cell.  Computes the average SCS-CN excess rate over
 * the interval [t_prev, t_now] and writes it to cell_source_mps.
 * The snapshot buffer (prev_cum_excess_mm) stores the cumulative excess
 * at the previous update tick; this kernel computes the incremental excess
 * over the interval and derives the average rate.
 * @global */
__global__ void swe2d_update_rain_source_rate_kernel(
    ...
    double* __restrict__ cum_rain_mm,
    double* __restrict__ cum_excess_mm,
    const double* __restrict__ prev_cum_excess_mm,
    double* __restrict__ cell_source_mps,
    double t_prev,
    double t_now,
    double ia_ratio,
    double mm_to_model_depth)
{
    ...
    const double r0 = interp_series_clamped_cuda(hg_time_s, hg_cum_mm, s, e, t_prev);
    const double r1 = interp_series_clamped_cuda(hg_time_s, hg_cum_mm, s, e, t_now);
    const double dr = fmax(0.0, r1 - r0);
    const double p_new = cum_rain_mm[c] + dr;
    ...
    double pe_new = 0.0;
    if (p_new > ia) {
        const double num = (p_new - ia) * (p_new - ia);
        const double den = fmax(p_new + (1.0 - ia_ratio) * s_mm, 1.0e-12);
        pe_new = num / den;
    }

    cum_rain_mm[c] = p_new;
    cum_excess_mm[c] = pe_new;

    const double dt_interval = fmax(t_now - t_prev, 1.0e-9);
    const double rate_mm_per_s = fmax(0.0, pe_new - prev_cum_excess_mm[c]) / dt_interval;
    cell_source_mps[c] = rate_mm_per_s * mm_to_model_depth;
}
...
__global__ __launch_bounds__(256, 2) void swe2d_flux_kernel(
    ...
    const double*  __restrict__ d_weno3_face_recon,
    const double*  __restrict__ d_mp5_face_recon,
    ...
    bool                        enable_shallow_front_recon_fallback)
{
    ...
    if (c1 >= 0) {
        ...
        const int scheme_bj     = static_cast<int>(SWE2DSpatialScheme::FV_BARTH_JESPERSEN);
        const int scheme_w3     = static_cast<int>(SWE2DSpatialScheme::FV_WENO3);
        const int scheme_weno5  = static_cast<int>(SWE2DSpatialScheme::FV_WENO5);
        const int scheme_m5     = static_cast<int>(SWE2DSpatialScheme::FV_MP5);
        ...
        if (!disable_higher_order && spatial_scheme >= scheme_fast && cell_cx != nullptr && d_grad != nullptr) {
            ...
            const double etaL = hL + zbL;
            const double etaR = hR + zbR;
            if (spatial_scheme == scheme_w3 && !near_boundary && d_weno3_face_recon != nullptr) {
                // WENO3 (scheme 6): read precomputed per-face value for eta; momentum first-order
                const double w3_val = d_weno3_face_recon[e];
                etaL_rec = w3_val - zbL;
                etaR_rec = w3_val - zbR;
                huL_rec = huL; huR_rec = huR;
                hvL_rec = hvL; hvR_rec = hvR;
            } else if (spatial_scheme == scheme_m5 && !near_boundary && d_mp5_face_recon != nullptr) {
                // MP5 (scheme 8): read precomputed per-face value for eta; momentum first-order
                const double m5_val = d_mp5_face_recon[e];
                etaL_rec = m5_val - zbL;
                etaR_rec = m5_val - zbR;
                huL_rec = huL; huR_rec = huR;
                hvL_rec = hvL; hvR_rec = hvR;
            ...
            } else if (spatial_scheme == scheme_bj && !near_boundary && d_grad_x_lim != nullptr) {
                // Barth-Jespersen (scheme 5): use limited eta gradients, unlimited momentum
                tvd_reconstruct(etaL, etaR, d_grad_x_lim[c0], d_grad_y_lim[c0], d_grad_x_lim[c1], d_grad_y_lim[c1], etaL_rec, etaR_rec);
 ...
 *  All input slopes are stored as actual f(t,y) values (NOT dt-scaled).
 *  @global */
    ...
    double h_min)
{
    ...
    const double h_new  = h0[c]  + dt * (k1_h[c]  + 2.0 * k2_h[c]  + 2.0 * k3_h[c]  + k4_h[c])  / 6.0;
    const double hu_new = hu0[c] + dt * (k1_hu[c] + 2.0 * k2_hu[c] + 2.0 * k3_hu[c] + k4_hu[c]) / 6.0;
    const double hv_new = hv0[c] + dt * (k1_hv[c] + 2.0 * k2_hv[c] + 2.0 * k3_hv[c] + k4_hv[c]) / 6.0;
    ...
    double           open_bc_relaxation)
{
    ...
    // ── Advanced spatial reconstruction stencil data (schemes 6, 8) ────────
    // WENO3 face sub-stencil CSR (scheme 6)
    if (!mesh.face_stencil_S0_offsets.empty()) {
        alloc_d(reinterpret_cast<void**>(&dev->d_face_stencil_S0_offsets),
                mesh.face_stencil_S0_offsets.size() * sizeof(int32_t));
        copy_h2d_i(dev->d_face_stencil_S0_offsets, mesh.face_stencil_S0_offsets.data(),
                   mesh.face_stencil_S0_offsets.size());
        alloc_d(reinterpret_cast<void**>(&dev->d_face_stencil_S0_cells),
                mesh.face_stencil_S0_cells.size() * sizeof(int32_t));
        copy_h2d_i(dev->d_face_stencil_S0_cells, mesh.face_stencil_S0_cells.data(),
                   mesh.face_stencil_S0_cells.size());
        alloc_d(reinterpret_cast<void**>(&dev->d_face_stencil_S1),
                mesh.face_stencil_S1.size() * sizeof(int32_t));
        copy_h2d_i(dev->d_face_stencil_S1, mesh.face_stencil_S1.data(),
                   mesh.face_stencil_S1.size());
        alloc_d(reinterpret_cast<void**>(&dev->d_face_stencil_S2_offsets),
                mesh.face_stencil_S2_offsets.size() * sizeof(int32_t));
        copy_h2d_i(dev->d_face_stencil_S2_offsets, mesh.face_stencil_S2_offsets.data(),
                   mesh.face_stencil_S2_offsets.size());
        alloc_d(reinterpret_cast<void**>(&dev->d_face_stencil_S2_cells),
                mesh.face_stencil_S2_cells.size() * sizeof(int32_t));
        copy_h2d_i(dev->d_face_stencil_S2_cells, mesh.face_stencil_S2_cells.data(),
                   mesh.face_stencil_S2_cells.size());
        dev->n_face_stencil_S0_cells = static_cast<int32_t>(mesh.face_stencil_S0_cells.size());
        dev->n_face_stencil_S2_cells = static_cast<int32_t>(mesh.face_stencil_S2_cells.size());
    }
    // MP5 5-cell walk data (scheme 8)
    if (!mesh.face_stencil_5.empty()) {
        alloc_d(reinterpret_cast<void**>(&dev->d_face_stencil_5),
                mesh.face_stencil_5.size() * sizeof(int32_t));
        copy_h2d_i(dev->d_face_stencil_5, mesh.face_stencil_5.data(),
                   mesh.face_stencil_5.size());
        alloc_d(reinterpret_cast<void**>(&dev->d_face_mp5_case),
                mesh.face_mp5_case.size() * sizeof(int32_t));
        copy_h2d_i(dev->d_face_mp5_case, mesh.face_mp5_case.data(),
                   mesh.face_mp5_case.size());
    }
    ...
    // Barth-Jespersen limited gradient arrays (scheme 5)
    alloc_d(reinterpret_cast<void**>(&dev->d_grad_x_lim), sz_cells * sizeof(double));
    alloc_d(reinterpret_cast<void**>(&dev->d_grad_y_lim), sz_cells * sizeof(double));
    ...
    // Face reconstruction precompute arrays (schemes 6, 8)
    alloc_d(reinterpret_cast<void**>(&dev->d_weno3_face_recon), sz_edges * sizeof(double));
    alloc_d(reinterpret_cast<void**>(&dev->d_mp5_face_recon),   sz_edges * sizeof(double));
    CUDA_CHECK(cudaMemset(dev->d_weno3_face_recon, 0, sz_edges * sizeof(double)));
    CUDA_CHECK(cudaMemset(dev->d_mp5_face_recon, 0, sz_edges * sizeof(double)));
    ...
    // k-slope buffers for graph-safe RK4/RK5/RK6 (temporal_order 4/5/6)
    ...
    CUDA_CHECK(cudaMemset(dev->d_k5_h,  0, sz_cells * sizeof(double)));
    CUDA_CHECK(cudaMemset(dev->d_k5_hu, 0, sz_cells * sizeof(double)));
    CUDA_CHECK(cudaMemset(dev->d_k5_hv, 0, sz_cells * sizeof(double)));
    ...
    // Intermediate state buffers (y_i momenta) for higher-order schemes
    ...
    bool   active_set_hysteresis)
{
    ...
    // Optional rain + CN forcing: re-evaluate SCS-CN rate at a fixed interval.
    // The source rate is held constant between updates; this reduces per-step
    // GPU kernel launches and eliminates the RK save/restore complexity.
    if (dev->n_rain_samples > 0 && dev->d_cell_gage_idx && dev->d_rain_hg_offsets && dev->d_rain_hg_time_s && dev->d_rain_hg_cum_mm && dev->d_rain_cn && dev->d_rain_excess_at_last_update_mm) {
        ...
        if (dev->last_rain_update_time < 0.0) {
            // First call ever: capture snapshot of current excess, record update time.
            // No rate is applied yet — next step will compute the first interval rate.
            ...
            CUDA_CHECK(cudaGetLastError());
            // After update: snapshot cum_excess_mm = state at t_now
            CUDA_CHECK(cudaMemcpyAsync(dev->d_rain_excess_at_last_update_mm,
                                     dev->d_rain_excess_cum_mm,
                                     sz_cells,
                                     cudaMemcpyDeviceToDevice,
        ...
        if (cache_match) {
            ...
            if (cap_begin == cudaSuccess) {
                ...
                // ── Advanced spatial reconstruction pre-passes (schemes 5, 6, 8) ──
                {
                    ...
                    // Scheme 5: extract eta gradients from Grad struct, apply Barth-Jespersen limiter
                    if (spatial_scheme == scheme_bj) {
                        extract_hx_hy_kernel<<<c_grid, BLOCK, 0, dev->d_stream>>>(
                            dev->d_grad, dev->d_grad_x_lim, dev->d_grad_y_lim, n_cells);
                        barth_jespersen_kernel<<<c_grid, BLOCK, 0, dev->d_stream>>>(
                            dev->d_h,
                    ...
                    // Scheme 6: precompute per-face WENO3 reconstruction
                    if (spatial_scheme == scheme_w3 && dev->d_face_stencil_S0_offsets != nullptr) {
                        CUDA_CHECK(cudaMemsetAsync(dev->d_weno3_face_recon, 0,
                            static_cast<size_t>(n_edges) * sizeof(double), dev->d_stream));
                        weno3_kernel<<<e_grid, BLOCK, 0, dev->d_stream>>>(
                            dev->d_h, dev->d_cell_cx, dev->d_cell_cy,
                            ...
                            n_edges, dev->d_weno3_face_recon);
                    }

                    // Scheme 8: precompute per-face MP5 reconstruction
                    if (spatial_scheme == scheme_m5 && dev->d_face_stencil_5 != nullptr) {
                        CUDA_CHECK(cudaMemsetAsync(dev->d_mp5_face_recon, 0,
                            static_cast<size_t>(n_edges) * sizeof(double), dev->d_stream));
                        mp5_kernel<<<e_grid, BLOCK, 0, dev->d_stream>>>(
                            dev->d_h, dev->d_cell_cx, dev->d_cell_cy,
                            dev->d_edge_mx, dev->d_edge_my,
                            dev->d_face_stencil_5, dev->d_face_mp5_case,
                            n_edges, dev->d_mp5_face_recon);
                    }
                }
                // flux
                ...
                swe2d_flux_kernel<<<grid_flux, BLOCK, 0, dev->d_stream>>>(
                    ...
                    dev->d_grad,
                    dev->d_grad_x_lim, dev->d_grad_y_lim,
                    dev->d_weno3_face_recon, dev->d_mp5_face_recon,
...
__global__ __launch_bounds__(256, 4) void swe2d_rk5_stage6_prep_kernel(
    int32_t n_cells,
    double dt,
    const double* k1_h,  const double* k1_hu,  const double* k1_hv,
    const double* k2_h,  const double* k2_hu,  const double* k2_hv,
    const double* k3_h,  const double* k3_hu,  const double* k3_hv,
    const double* k4_h,  const double* k4_hu,  const double* k4_hv,
    const double* k5_h,  const double* k5_hu,  const double* k5_hv,
    double a1, double a2, double a3, double a4, double a5,
    double* h, double* hu, double* hv)
{
    int32_t c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= n_cells) return;
    h[c]  += dt * (a1 * k1_h[c]  + a2 * k2_h[c]  + a3 * k3_h[c]  + a4 * k4_h[c]  + a5 * k5_h[c]);
    hu[c] += dt * (a1 * k1_hu[c] + a2 * k2_hu[c] + a3 * k3_hu[c] + a4 * k4_hu[c] + a5 * k5_hu[c]);
    hv[c] += dt * (a1 * k1_hv[c] + a2 * k2_hv[c] + a3 * k3_hv[c] + a4 * k4_hv[c] + a5 * k5_hv[c]);
}
...
void swe2d_gpu_step_rk5(
    ...
    bool active_set_hysteresis)
{
    // Cash-Karp RK5(4) embedded (graph-safe).
    //
    // Coefficients (standard Cash-Karp, Numerical Recipes / Wikipedia):
    ...
    //
    // 5th-order combine weights: b1=37/378 b3=250/621 b4=125/594 b6=512/1771
    // (b2 = b5 = 0; k2 and k5 used only by intermediate stages, NOT in combine.)
    //
    // Slope storage plan (each preserved until combine reads it):
    //   k1 -> d_k4_h/hu/hv
    //   k2 -> d_k5_h/hu/hv
    //   k3 -> d_k6_h/hu/hv
    //   k4 -> d_h1/hu1/hv1
    ...
    //   k6 -> d_h3/hu3/hv3
    //
    // y_i intermediate-state scratch (saved BEFORE Stage i GPU step, then reused):
    //   Stage 2: y2 (State) -> d_h1/hu1/hv1, then later overwritten by k4 after Stage 4.
    //   Stage 3: y3 (State) -> d_h2/hu2/hv2, then later overwritten by k5 after Stage 5.
    //   Stage 4: y4 (State) -> d_h3/hu3/hv3, then later overwritten by k6 after Stage 6.
    //   Stage 5: y5 (State) -> d_h3/hu3/hv3 (re-using the buffer since y4 not needed).
    //   Stage 6: y6 (State) -> d_h3/hu3/hv3 (re-using the buffer since y5 not needed).
    ...
    // Save base state y0 to d_h0/hu0/hv0 (double-precision).
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h0);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu0);
    ...
    // ========================================================================
    // Stage 1: k1 = f(t, y0); h1 = y0 + dt*k1.
    // Compute k1 = (double)h1 - y0 via state_subtract_double_kernel.
    // ========================================================================
    swe2d_gpu_step(dev, t_now, dt, g, h_min, spatial_scheme, cfl_factor,
                   max_inv_area, cfl_lambda_cap,
                   momentum_cap_min_speed, momentum_cap_celerity_mult,
    ...
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h,  dev->d_h0,  dev->d_k4_h);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
    ...
    // y2 scratch -> d_h1/hu1/hv1 (State). k2 -> d_k5_h/hu/hv.
    // ========================================================================
    swe2d_rk5_stage2_intermediate_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dt,
        dev->d_h0, dev->d_hu0, dev->d_hv0,
        dev->d_k4_h, dev->d_k4_hu, dev->d_k4_hv,
        dev->d_h, dev->d_hu, dev->d_hv);
    // Save y2 (State) to d_h1/hu1/hv1 (as double) BEFORE the GPU step overwrites d_h.
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
    ...
    swe2d_gpu_step(dev, t_now + dt * (1.0 / 5.0), dt, g, h_min, spatial_scheme, cfl_factor,
                   ...
                   false, &tmp_diag,
                   front_flux_damping, active_set_hysteresis);
    // Now d_h = h2 (State). k2 = (double)h2 - y2.
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h,  dev->d_h1,  dev->d_k5_h);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu1, dev->d_k5_hu);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv1, dev->d_k5_hv);

    // ========================================================================
    // Stage 3: y3 = y0 + dt*(a31*k1 + a32*k2); a31=3/40, a32=9/40.
    // Restore y0 to d_h. Apply prep kernel. Save y3 as State. Stage 3 GPU step.
    // y3 scratch -> d_h2/hu2/hv2 (State). k3 -> d_k6_h/hu/hv.
    // ========================================================================
    ...
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hv, dev->d_hv0, sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    {
        ...
        const double a1 = 3.0 / 40.0;
        const double a2 = 9.0 / 40.0;
        ...
        const double* __restrict__ k2h  = dev->d_k5_h;
        const double* __restrict__ k2hu = dev->d_k5_hu;
        const double* __restrict__ k2hv = dev->d_k5_hv;
    ...
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h2);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu2);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv2);
    ...
    // k3 = h3 - y3.
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h,  dev->d_h2,  dev->d_k6_h);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu2, dev->d_k6_hu);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv2, dev->d_k6_hv);

    // ========================================================================
    // Stage 4: y4 = y0 + dt*(a41*k1 + a42*k2 + a43*k3); a41=3/10, a42=-9/10, a43=6/5.
    // y4 scratch -> d_h3/hu3/hv3 (State). k4 -> d_h1/hu1/hv1 (overwriting stale y2).
    // ========================================================================
    CUDA_CHECK(cudaMemcpyAsync(dev->d_h,  dev->d_h0,  sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hu, dev->d_hu0, sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hv, dev->d_hv0, sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    ...
    // Save y4 (State) to d_h3/hu3/hv3.
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h3);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu3);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv3);
    swe2d_gpu_step(dev, t_now + dt * (3.0 / 5.0), dt, g, h_min, spatial_scheme, cfl_factor,
                   ...
                   false, &tmp_diag,
                   front_flux_damping, active_set_hysteresis);
    // k4 = h4 - y4 -> d_h1/hu1/hv1.
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h,  dev->d_h3,  dev->d_h1);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu3, dev->d_hu1);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv3, dev->d_hv1);

    // ========================================================================
    // Stage 5: y5 = y0 + dt*(a51*k1 + a52*k2 + a53*k3 + a54*k4);
    //   a51=-11/54, a52=5/2, a53=-70/27, a54=35/27.
    // y5 scratch -> d_h3/hu3/hv3 (re-using slot since y4 not needed post Stage 5).
    // k5 -> d_h2/hu2/hv2 (overwriting stale y3).
    // ========================================================================
    CUDA_CHECK(cudaMemcpyAsync(dev->d_h,  dev->d_h0,  sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hu, dev->d_hu0, sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hv, dev->d_hv0, sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    {
        const int n = n_cells;
        const double dt_val = dt;
        const double a1 = -11.0 / 54.0;
        const double a2 =    5.0 /  2.0;
        const double a3 = -70.0 / 27.0;
        const double a4 =  35.0 / 27.0;
        const double* __restrict__ k1h  = dev->d_k4_h;
        const double* __restrict__ k1hu = dev->d_k4_hu;
        const double* __restrict__ k1hv = dev->d_k4_hv;
        const double* __restrict__ k2h  = dev->d_k5_h;
    ...
    swe2d_gpu_step(dev, t_now + dt, dt, g, h_min, spatial_scheme, cfl_factor,
                   ...
                   false, &tmp_diag,
                   front_flux_damping, active_set_hysteresis);
    // k5 = h5 - y5 -> d_h2/hu2/hv2.
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h,  dev->d_h3,  dev->d_h2);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu3, dev->d_hu2);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv3, dev->d_hv2);

    // ========================================================================
    // Stage 6: y6 = y0 + dt*(a61*k1 + a62*k2 + a63*k3 + a64*k4 + a65*k5);
    //   a61=1631/55296, a62=175/512, a63=575/13824, a64=44275/110592, a65=253/4096.
    // y6 scratch -> d_h3/hu3/hv3 (re-using slot since y5 not needed post Stage 6).
    // k6 -> d_h3/hu3/hv3 (overwrites y6 scratch; this is the last operation).
    // ========================================================================
    CUDA_CHECK(cudaMemcpyAsync(dev->d_h,  dev->d_h0,  sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hu, dev->d_hu0, sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hv, dev->d_hv0, sz, cudaMemcpyDeviceToDevice, dev->d_stream));
    {
        const int n = n_cells;
        const double dt_val = dt;
        const double a1 =     1631.0 / 55296.0;
        const double a2 =      175.0 /   512.0;
        const double a3 =      575.0 / 13824.0;
        const double a4 =    44275.0 /110592.0;
        const double a5 =      253.0 /  4096.0;
        const double* __restrict__ k1h  = dev->d_k4_h;
        const double* __restrict__ k1hu = dev->d_k4_hu;
        const double* __restrict__ k1hv = dev->d_k4_hv;
        const double* __restrict__ k2h  = dev->d_k5_h;
        ...
        const double* __restrict__ k4hu = dev->d_hu1;
        const double* __restrict__ k4hv = dev->d_hv1;
        const double* __restrict__ k5h  = dev->d_h2;
        const double* __restrict__ k5hu = dev->d_hu2;
        const double* __restrict__ k5hv = dev->d_hv2;
        double* __restrict__ h_d  = dev->d_h;
        double* __restrict__ hu_d = dev->d_hu;
        double* __restrict__ hv_d = dev->d_hv;
        swe2d_rk5_stage6_prep_kernel<<<(n + BLOCK - 1) / BLOCK, BLOCK, 0, dev->d_stream>>>(
            n, dt_val,
            k1h, k1hu, k1hv,
            k2h, k2hu, k2hv,
            k3h, k3hu, k3hv,
            k4h, k4hu, k4hv,
            k5h, k5hu, k5hv,
            a1, a2, a3, a4, a5, h_d, hu_d, hv_d);
    }
    // Save y6 (State) to d_h3/hu3/hv3 (overwrites stale k5 was already done by Stage 5
    // saving to d_h2; d_h3 still has whatever was last written there, which is unused,
    // so this is safe).
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h3);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
    ...
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv3);
    swe2d_gpu_step(dev, t_now + dt * (7.0 / 8.0), dt, g, h_min, spatial_scheme, cfl_factor,
                   max_inv_area, cfl_lambda_cap,
                   momentum_cap_min_speed, momentum_cap_celerity_mult,
                   depth_cap, max_rel_depth_increase, shallow_damping_depth,
                   source_cfl_beta, source_max_substeps,
                   ...
                   false, &tmp_diag,
                   front_flux_damping, active_set_hysteresis);
    // k6 = h6 - y6 -> d_h3/hu3/hv3 (overwrites y6).
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h,  dev->d_h3,  dev->d_h3);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu3, dev->d_hu3);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv3, dev->d_hv3);

    // ========================================================================
    // Combine: y_new = y0 + dt*(b1*k1 + b3*k3 + b4*k4 + b6*k6)
    //   b1 = 37/378, b3 = 250/621, b4 = 125/594, b6 = 512/1771 (5th-order)
    // k1 -> d_k4_*,  k3 -> d_k6_*,  k4 -> d_h1/hu1/hv1,  k6 -> d_h3/hu3/hv3
    // Combine kernel also applies friction, momentum cap, wet/dry threshold.
    // ========================================================================
    CUDA_CHECK(cudaMemsetAsync(dev->d_max_wse_elev_error, 0, sizeof(double), dev->d_stream));
    const int grid = (n_cells + BLOCK - 1) / BLOCK;
    swe2d_rk5_graph_combine_kernel<<<grid, BLOCK, 0, dev->d_stream>>>(
        n_cells,
        dev->d_h, dev->d_hu, dev->d_hv,
        dev->d_h0, dev->d_hu0, dev->d_hv0,
        dev->d_k4_h, dev->d_k4_hu, dev->d_k4_hv,
...
void swe2d_gpu_step_rk4(
    ...
    bool active_set_hysteresis)
{
    // Textbook RK4 (graph-safe). Each stage captures its own graph.
    //
    // Slope storage (all stored as actual slopes k_i = f(t, y_i), NOT dt-scaled):
    //   d_k4: k1 = f(t, y0)
    //   d_k5: k2 = f(t+dt/2, y2_state)
    //   d_k6: k3 = f(t+dt/2, y3_state)
    //   d_h1: k4 = f(t+dt, y4_state)
    //
    // Combine: y_new = y0 + dt*(k1 + 2*k2 + 2*k3 + k4)/6
    //
    // For consistency with the prep kernels' "h += dt*a*k_input" convention, we store the
    // 4 slopes at the SAME convention so the prep kernels and the combine agree.
    //
    // Implementation: stage i stores k_i = (h_i_full_state - y_i_state)/dt_i
    //   Stage 1: dt_i = dt,              k1 = (h1 - y0)/dt
    //   Stage 2: dt_i = dt/2,            k2 = (h2 - y2_state)/(dt/2) = 2*(h2-y2_state)/dt
    //   Stage 3: dt_i = dt/2,            k3 = (h3 - y3_state)/(dt/2) = 2*(h3-y3_state)/dt
    //   Stage 4: dt_i = dt,              k4 = (h4 - y4_state)/dt
    //
    // These are all f(t,y) values (units: d/dt). Combine weights (1, 2, 2, 1)/6 multiplied by
    // dt give the textbook RK4 update.
    ...
    // ========================================================================
    // Stage 1: k1 = f(t, y0); h1 = y0 + dt*k1.
    // Save k1_actual = (h1 - y0)/dt  to d_k4.
    // ========================================================================
    swe2d_gpu_step(dev, t_now, dt, g, h_min, spatial_scheme, cfl_factor,
                   max_inv_area, cfl_lambda_cap,
                   momentum_cap_min_speed, momentum_cap_celerity_mult,
                   ...
                   false, &tmp_diag,
                   front_flux_damping, active_set_hysteresis);
    // k1 = h1 - y0  (raw diff, divided by dt next)
    swe2d_double_sub_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h0, dev->d_k4_h);
    swe2d_double_sub_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu0, dev->d_k4_hu);
    swe2d_double_sub_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv0, dev->d_k4_hv);
    // d_k4 *= 1/dt -> d_k4 = k1_actual
    const double inv_dt = 1.0 / dt;
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, inv_dt, dev->d_k4_h);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, inv_dt, dev->d_k4_hu);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
    ...
    // ========================================================================
    swe2d_rk4_stage2_intermediate_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dt,
        dev->d_h0, dev->d_hu0, dev->d_hv0,
        dev->d_k4_h, dev->d_k4_hu, dev->d_k4_hv,  // k1 actual
        dev->d_h, dev->d_hu, dev->d_hv);
    // Save y2_state to d_h2/hu2/hv2 (as double).
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h2);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu2);
    ...
    swe2d_gpu_step(dev, t_now + dt * 0.5, dt * 0.5, g, h_min, spatial_scheme, cfl_factor,
                   ...
                   false, &tmp_diag,
                   front_flux_damping, active_set_hysteresis);
    // d_h state = h2. d_h2 double = y2_state.
    // k2 = 2*(h2 - y2_state)/dt.
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h2, dev->d_k5_h);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
    ...
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv2, dev->d_k5_hv);
    // d_k5 = (h2 - y2_state) = (dt/2)*k2_actual. Scale by 2/dt -> k2_actual.
    const double two_over_dt = 2.0 / dt;
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, two_over_dt, dev->d_k5_h);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, two_over_dt, dev->d_k5_hu);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, two_over_dt, dev->d_k5_hv);

    // ========================================================================
    // Stage 3: y3_state = y0 + (dt/2)*k2. Step at t+dt/2 dt/2 -> h3.
    //   k3 = 2*(h3 - y3_state)/dt -> d_k6
    // ========================================================================
    // Restore d_h = y0 (since prep does y0 += (dt/2)*k2).
    CUDA_CHECK(cudaMemcpyAsync(dev->d_h,  dev->d_h0,  static_cast<size_t>(n_cells) * sizeof(double), cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hu, dev->d_hu0, static_cast<size_t>(n_cells) * sizeof(double), cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hv, dev->d_hv0, static_cast<size_t>(n_cells) * sizeof(double), cudaMemcpyDeviceToDevice, dev->d_stream));
    {
        const int n = n_cells;
        const double dt_val = dt;
        const double a1 = 1.0 / 2.0;  // (dt/2)
        const double* __restrict__ k2_h  = dev->d_k5_h;
        const double* __restrict__ k2_hu = dev->d_k5_hu;
        const double* __restrict__ k2_hv = dev->d_k5_hv;
        double* __restrict__ h_d  = dev->d_h;
        double* __restrict__ hu_d = dev->d_hu;
        double* __restrict__ hv_d = dev->d_hv;
        swe2d_rk5_stage34_prep_kernel<<<(n + BLOCK - 1) / BLOCK, BLOCK, 0, dev->d_stream>>>(
            n, dt_val, k2_h, k2_hu, k2_hv, k2_h, k2_hu, k2_hv, k2_h, k2_hu, k2_hv,
            a1, 0.0, 0.0, h_d, hu_d, hv_d);
    }
    // Save y3_state to d_h3/hu3/hv3.
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h3);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu3);
    ...
    // k3 = 2*(h3 - y3_state)/dt.
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h3, dev->d_k6_h);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu3, dev->d_k6_hu);
    swe2d_state_subtract_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv3, dev->d_k6_hv);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, two_over_dt, dev->d_k6_h);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, two_over_dt, dev->d_k6_hu);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, two_over_dt, dev->d_k6_hv);

    // ========================================================================
    // Stage 4: y4_state = y0 + dt*k3. Step at t+dt dt -> h4.
    //   k4 = (h4 - y4_state)/dt -> d_h1
    //   (NOTE: a1=1.0 since a4=1 for RK4 Stage 4, NOT the Cash-Karp value)
    // ========================================================================
    CUDA_CHECK(cudaMemcpyAsync(dev->d_h,  dev->d_h0,  static_cast<size_t>(n_cells) * sizeof(double), cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hu, dev->d_hu0, static_cast<size_t>(n_cells) * sizeof(double), cudaMemcpyDeviceToDevice, dev->d_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev->d_hv, dev->d_hv0, static_cast<size_t>(n_cells) * sizeof(double), cudaMemcpyDeviceToDevice, dev->d_stream));
    {
        const int n = n_cells;
        const double dt_val = dt;
        const double a1 = 1.0;  // (dt/1) * k3  →  y4_state = y0 + dt*k3
        const double* __restrict__ k3_h  = dev->d_k6_h;
        const double* __restrict__ k3_hu = dev->d_k6_hu;
        const double* __restrict__ k3_hv = dev->d_k6_hv;
        double* __restrict__ h_d  = dev->d_h;
        double* __restrict__ hu_d = dev->d_hu;
        double* __restrict__ hv_d = dev->d_hv;
        swe2d_rk5_stage34_prep_kernel<<<(n + BLOCK - 1) / BLOCK, BLOCK, 0, dev->d_stream>>>(
            n, dt_val, k3_h, k3_hu, k3_hv, k3_h, k3_hu, k3_hv, k3_h, k3_hu, k3_hv,
            a1, 0.0, 0.0, h_d, hu_d, hv_d);
    }
    // Save y4_state to d_h2/hu2/hv2 (reusing slot — y3 scratch was overwritten).
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h2);
    swe2d_state_to_double_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu2);
    ...
    // k4 = (h4 - y4_state)/dt.
    swe2d_double_sub_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_h, dev->d_h2, dev->d_h1);
    swe2d_double_sub_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hu, dev->d_hu2, dev->d_hu1);
    swe2d_double_sub_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, dev->d_hv, dev->d_hv2, dev->d_hv1);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, inv_dt, dev->d_h1);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, inv_dt, dev->d_hu1);
    swe2d_double_scale_inplace_kernel<<<grid_cells, BLOCK, 0, dev->d_stream>>>(
        n_cells, inv_dt, dev->d_hv1);
    ...
    swe2d_rk4_combine_kernel<<<grid, BLOCK, 0, dev->d_stream>>>(
        ...
        dev->d_h0, dev->d_hu0, dev->d_hv0,
        dev->d_k4_h, dev->d_k4_hu, dev->d_k4_hv,    // k1 actual
        dev->d_k5_h, dev->d_k5_hu, dev->d_k5_hv,    // k2 actual
        dev->d_k6_h, dev->d_k6_hu, dev->d_k6_hv,    // k3 actual
        dev->d_h1, dev->d_hu1, dev->d_hv1,          // k4 actual
        dt,
        dev->d_max_wse_elev_error,
        h_min);
    CUDA_CHECK(cudaGetLastError());
    ...
    double rain_update_interval_s)
{
    ...
    if (dev->d_rain_excess_at_last_update_mm) CUDA_CHECK(cudaFree(dev->d_rain_excess_at_last_update_mm));
    ...
    dev->d_rain_excess_at_last_update_mm = nullptr;
    dev->n_rain_gages = 0;
    dev->n_rain_samples = 0;
    dev->rain_ia_ratio = ia_ratio;
    dev->rain_mm_to_model_depth = (mm_to_model_depth > 0.0) ? mm_to_model_depth : 1.0e-3;
    dev->rain_update_interval_s = rain_update_interval_s;
    dev->last_rain_update_time = -1.0;
    ...
    if (dev->n_cells > 0) {
        if (dev->d_rain_excess_at_last_update_mm) CUDA_CHECK(cudaFree(dev->d_rain_excess_at_last_update_mm));
        CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&dev->d_rain_excess_at_last_update_mm), static_cast<size_t>(dev->n_cells) * sizeof(double)));
        CUDA_CHECK(cudaMemset(dev->d_rain_excess_at_last_update_mm, 0, static_cast<size_t>(dev->n_cells) * sizeof(double)));
    }
...
// ── pipe1d code moved to pipe1d.cu ─────────────────────────────────────────
...
void swe2d_gpu_destroy(SWE2DDeviceState* dev) {
    ...
    safe_free(dev->d_weno3_face_recon); safe_free(dev->d_mp5_face_recon);
    ...
    safe_free(dev->d_face_stencil_5); safe_free(dev->d_face_mp5_case);
    ...
    safe_free(dev->d_rain_excess_at_last_update_mm);
    ...
    safe_free(dev->d_k5_h);  safe_free(dev->d_k5_hu); safe_free(dev->d_k5_hv);
    safe_free(dev->d_k6_h);  safe_free(dev->d_k6_hu); safe_free(dev->d_k6_hv);
    safe_free(dev->d_h1);   safe_free(dev->d_hu1);  safe_free(dev->d_hv1);
    safe_free(dev->d_h2);   safe_free(dev->d_hu2);  safe_free(dev->d_hv2);
